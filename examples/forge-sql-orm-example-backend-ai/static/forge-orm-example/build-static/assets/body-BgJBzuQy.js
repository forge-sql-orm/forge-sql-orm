import { a as e } from "./rolldown-runtime-Cyuzqnbw.js";
import { n as t, r as n } from "./react-dom-vendor-CRtau6sw.js";
import {
  A as r,
  C as i,
  D as a,
  E as o,
  O as s,
  S as c,
  T as l,
  b as u,
  d,
  f,
  g as p,
  h as m,
  k as h,
  m as g,
  n as _,
  p as v,
  r as y,
  t as b,
  u as x,
  v as S,
  w as C,
  x as w,
  y as T,
} from "./index-B6uUGBBW.js";
function E(e, t, n) {
  return ((t = u(t)), w(e, D() ? Reflect.construct(t, n || [], u(e).constructor) : t.apply(e, n)));
}
function D() {
  try {
    var e = !Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {}));
  } catch {}
  return (D = function () {
    return !!e;
  })();
}
var O = !0,
  k = `Invariant failed`,
  A = (function (e) {
    function t(e) {
      var n;
      return (l(this, t), (n = E(this, t)), (n.message = e), n);
    }
    return (
      T(t, e),
      C(t, [
        {
          key: `toString`,
          value: function () {
            return this.message;
          },
        },
      ])
    );
  })(S(Error));
function j(e, t) {
  if (!e) throw O ? new A(k) : new A(`${k}: ${t || ``}`);
}
var M = e(n()),
  N = (0, M.createContext)(null);
function P() {
  var e = (0, M.useContext)(N);
  return (j(e !== null, `Unable to find DragDropContext context`), e);
}
function ee(e) {
  var t = e.children,
    n = e.contextId,
    r = e.getDragState,
    i = e.startKeyboardDrag,
    a = e.droppableRegistry,
    o = (0, M.useMemo)(
      function () {
        return { contextId: n, getDragState: r, startKeyboardDrag: i, droppableRegistry: a };
      },
      [n, r, i, a],
    );
  return M.createElement(N.Provider, { value: o }, t);
}
function F(e) {
  return { margin: window.getComputedStyle(e).margin, rect: e.getBoundingClientRect() };
}
function I() {
  var e = P().getDragState,
    t = e();
  return t.isDragging ? t.draggableDimensions : null;
}
var te = function () {};
function ne() {
  var e = te;
  return {
    setCleanupFn: function (t) {
      e = t;
    },
    runCleanupFn: function () {
      (e(), (e = te));
    },
  };
}
function re() {
  var e = h((0, M.useState)(ne), 1)[0];
  return (
    (0, M.useEffect)(
      function () {
        return e.runCleanupFn;
      },
      [e.runCleanupFn],
    ),
    e
  );
}
var L = {
    draggable: { contextId: `data-rbd-draggable-context-id`, id: `data-rbd-draggable-id` },
    dragHandle: {
      contextId: `data-rbd-drag-handle-context-id`,
      draggableId: `data-rbd-drag-handle-draggable-id`,
    },
    droppable: { contextId: `data-rbd-droppable-context-id`, id: `data-rbd-droppable-id` },
    placeholder: { contextId: `data-rbd-placeholder-context-id` },
  },
  R = {
    draggable: {
      droppableId: `data-rbd-draggable-droppable-id`,
      index: `data-rbd-draggable-index`,
    },
    dropIndicator: `data-rbd-drop-indicator`,
    droppable: { direction: `data-rbd-droppable-direction`, type: `data-rbd-droppable-type` },
  };
function z(e, t) {
  var n = e.getAttribute(t);
  return (j(n !== null, `Expected '${t}' to be present`), n);
}
function ie(e, t) {
  for (var n = 0, r = Object.entries(t); n < r.length; n++) {
    var i = h(r[n], 2),
      a = i[0],
      o = i[1];
    e.setAttribute(a, o);
  }
  return function () {
    for (var n = 0, r = Object.keys(t); n < r.length; n++) {
      var i = r[n];
      e.removeAttribute(i);
    }
  };
}
function ae() {
  return [...arguments]
    .map(function (e) {
      var t = e.attribute,
        n = e.value;
      return n ? `[${t}="${CSS.escape(n)}"]` : `[${t}]`;
    })
    .join(``);
}
function B() {
  var e = ae.apply(void 0, arguments);
  return document.querySelector(e);
}
function oe() {
  var e = ae.apply(void 0, arguments);
  return Array.from(document.querySelectorAll(e));
}
function V() {
  var e = B.apply(void 0, arguments);
  return (j(e, `There is a matching HTMLElement for selector ` + ae.apply(void 0, arguments)), e);
}
function se(e) {
  var t = e.contextId,
    n = e.draggableId;
  return B(
    { attribute: L.dragHandle.contextId, value: t },
    { attribute: L.dragHandle.draggableId, value: n },
  );
}
function ce(e) {
  if (getComputedStyle(e).position === `absolute`) return e;
  var t = e.parentElement;
  return t instanceof HTMLElement ? ce(t) : null;
}
function le(e) {
  var t = e.element;
  return e.mode === `standard` ? t : (ce(t) ?? t);
}
function ue() {
  window.dispatchEvent(new DragEvent(`dragend`));
}
function de(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function fe(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? de(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : de(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var pe = {
    top: function (e, t) {
      return Math.abs(t.y - e.top);
    },
    right: function (e, t) {
      return Math.abs(e.right - t.x);
    },
    bottom: function (e, t) {
      return Math.abs(e.bottom - t.y);
    },
    left: function (e, t) {
      return Math.abs(t.x - e.left);
    },
  },
  me = Symbol(`closestEdge`);
function he(e, t) {
  var n = t.element,
    r = t.input,
    a = t.allowedEdges,
    o = { x: r.clientX, y: r.clientY },
    s = n.getBoundingClientRect(),
    c =
      a
        .map(function (e) {
          return { edge: e, value: pe[e](s, o) };
        })
        .sort(function (e, t) {
          return e.value - t.value;
        })[0]?.edge ?? null;
  return fe(fe({}, e), {}, i({}, me, c));
}
function ge(e) {
  return e[me] ?? null;
}
var _e = Symbol(`DraggableData`);
function H(e) {
  return e[_e] === !0;
}
function ve(e) {
  var t = e.draggableId,
    n = e.droppableId,
    r = e.getIndex,
    a = e.contextId,
    o = e.type;
  return (0, M.useMemo)(
    function () {
      return i(
        i(
          i(i(i(i({}, _e, !0), `draggableId`, t), `droppableId`, n), `getIndex`, r),
          `contextId`,
          a,
        ),
        `type`,
        o,
      );
    },
    [t, n, r, a, o],
  );
}
var ye = Symbol(`DroppableData`);
function be(e) {
  return e[ye] === !0;
}
function xe(e) {
  var t = e.contextId,
    n = e.droppableId,
    r = e.getIsDropDisabled;
  return (0, M.useMemo)(
    function () {
      return i(i(i(i({}, ye, !0), `contextId`, t), `droppableId`, n), `getIsDropDisabled`, r);
    },
    [t, n, r],
  );
}
function Se(e) {
  var t = e.droppableId,
    n = e.contextId;
  return oe(
    { attribute: L.draggable.contextId, value: n },
    { attribute: R.draggable.droppableId, value: t },
  );
}
var Ce = [`droppableId`, `getIndex`],
  we = [`contextId`, `droppableId`];
function Te(e) {
  var t = e.droppableId,
    n = e.getIndex,
    r = s(e, Ce),
    i = n(),
    a = ge(r);
  return ((a === `bottom` || a === `right`) && (i += 1), { droppableId: t, index: i });
}
function Ee(e) {
  var t = e.contextId,
    n = e.droppableId,
    r = s(e, we),
    i = Se({ contextId: t, droppableId: n });
  if (i.length === 0) return { droppableId: n, index: 0 };
  var a = ge(r);
  return a === `top` || a === `left`
    ? { droppableId: n, index: 0 }
    : {
        droppableId: n,
        index:
          i.reduce(function (e, t) {
            var n = parseInt(z(t, R.draggable.index), 10);
            return Math.max(e, n);
          }, 0) + 1,
      };
}
function De(e) {
  var t = e.dropTargets;
  if (t.length === 0) return null;
  var n = t[0];
  return H(n.data) ? Te(n.data) : be(n.data) ? Ee(n.data) : null;
}
function Oe(e, t) {
  return !(e?.droppableId !== t?.droppableId || e?.index !== t?.index);
}
function ke() {
  var e = new Map(),
    t = function (t) {
      var n = t.droppableId;
      return e.get(n) ?? null;
    },
    n = null;
  return {
    getEntry: t,
    register: function (t) {
      var r;
      return (
        e.set(t.droppableId, t),
        (r = n) == null || r(t),
        function () {
          e.delete(t.droppableId);
        }
      );
    },
    setUpdateListener: function (e) {
      n = e;
    },
  };
}
function Ae() {
  return h((0, M.useState)(ke), 1)[0];
}
function U() {
  var e = [...arguments];
  return function () {
    e.forEach(function (e) {
      return e();
    });
  };
}
var je = `data-pdnd-honey-pot`;
function Me(e) {
  return e instanceof Element && e.hasAttribute(`data-pdnd-honey-pot`);
}
function Ne(e) {
  var t = h(document.elementsFromPoint(e.x, e.y), 2),
    n = t[0],
    r = t[1];
  return n ? (Me(n) ? (r ?? null) : n) : null;
}
var Pe = 2147483647,
  Fe = {
    inset: `unset`,
    border: `none`,
    padding: 0,
    margin: 0,
    overflow: `visible`,
    color: `inherit`,
    background: `transparent`,
  };
function W(e) {
  var t = null;
  return function () {
    if (!t) {
      var n = [...arguments];
      t = { result: e.apply(this, n) };
    }
    return t.result;
  };
}
var Ie = W(function () {
    return typeof HTMLElement < `u` && typeof HTMLElement.prototype.showPopover == `function`;
  }),
  G = x();
function Le(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function Re(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? Le(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : Le(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var ze = 2,
  Be = ze / 2;
function Ve(e) {
  return { x: Math.floor(e.x), y: Math.floor(e.y) };
}
function He(e) {
  return { x: e.x - Be, y: e.y - Be };
}
function Ue(e) {
  return { x: Math.max(e.x, 0), y: Math.max(e.y, 0) };
}
function We(e) {
  return { x: Math.min(e.x, window.innerWidth - ze), y: Math.min(e.y, window.innerHeight - ze) };
}
function Ge(e) {
  var t = e.client,
    n = We(Ue(He(Ve(t))));
  return DOMRect.fromRect({ x: n.x, y: n.y, width: ze, height: ze });
}
function Ke(e) {
  var t = e.clientRect;
  return { left: `${t.left}px`, top: `${t.top}px`, width: `${t.width}px`, height: `${t.height}px` };
}
function qe(e) {
  var t = e.client,
    n = e.clientRect;
  return t.x >= n.x && t.x <= n.x + n.width && t.y >= n.y && t.y <= n.y + n.height;
}
function Je(e) {
  var t = e.initial,
    n = document.createElement(`div`);
  (n.setAttribute(je, `true`), Ie() && n.setAttribute(`popover`, `manual`));
  var r = Ge({ client: t });
  (Object.assign(
    n.style,
    Re(
      Re({ position: `fixed` }, Ie() ? Fe : { zIndex: Pe }),
      {},
      {
        backgroundColor: `transparent`,
        padding: 0,
        margin: 0,
        boxSizing: `border-box`,
        pointerEvents: `auto`,
      },
      Ke({ clientRect: r }),
    ),
  ),
    document.body.appendChild(n),
    Ie() && n.showPopover());
  var i = (0, G.bind)(window, {
    type: `pointermove`,
    listener: function (e) {
      ((r = Ge({ client: { x: e.clientX, y: e.clientY } })),
        Object.assign(n.style, Ke({ clientRect: r })));
    },
    options: { capture: !0 },
  });
  return function (e) {
    var t = e.current;
    if ((i(), qe({ client: t, clientRect: r }))) {
      n.remove();
      return;
    }
    function a() {
      (o(), n.remove());
    }
    var o = (0, G.bindAll)(
      window,
      [
        { type: `pointerdown`, listener: a },
        { type: `pointermove`, listener: a },
        { type: `focusin`, listener: a },
        { type: `focusout`, listener: a },
        { type: `dragstart`, listener: a },
        { type: `dragenter`, listener: a },
        { type: `dragover`, listener: a },
      ],
      { capture: !0 },
    );
  };
}
function Ye() {
  var e = null;
  function t() {
    return (
      (e = null),
      (0, G.bind)(window, {
        type: `pointermove`,
        listener: function (t) {
          e = { x: t.clientX, y: t.clientY };
        },
        options: { capture: !0 },
      })
    );
  }
  function n() {
    var t = null;
    return function (n) {
      var r = n.eventName,
        i = n.payload;
      if (r === `onDragStart`) {
        var a = i.location.initial.input;
        t = Je({ initial: e ?? { x: a.clientX, y: a.clientY } });
      }
      if (r === `onDrop`) {
        var o,
          s = i.location.current.input;
        ((o = t) == null || o({ current: { x: s.clientX, y: s.clientY } }), (t = null), (e = null));
      }
    };
  }
  return { bindEvents: t, getOnPostDispatch: n };
}
var Xe = W(function () {
    return navigator.userAgent.includes(`Firefox`);
  }),
  Ze = W(function () {
    var e = navigator.userAgent;
    return e.includes(`AppleWebKit`) && !e.includes(`Chrome`);
  }),
  Qe = { isLeavingWindow: Symbol(`leaving`), isEnteringWindow: Symbol(`entering`) };
function $e(e) {
  var t = e.dragLeave;
  return Ze() ? t.hasOwnProperty(Qe.isLeavingWindow) : !1;
}
(function () {
  if (typeof window > `u` || !Ze()) return;
  function e() {
    return { enterCount: 0, isOverWindow: !1 };
  }
  var t = e();
  function n() {
    t = e();
  }
  (0, G.bindAll)(
    window,
    [
      {
        type: `dragstart`,
        listener: function () {
          ((t.enterCount = 0), (t.isOverWindow = !0));
        },
      },
      { type: `drop`, listener: n },
      { type: `dragend`, listener: n },
      {
        type: `dragenter`,
        listener: function (e) {
          (!t.isOverWindow && t.enterCount === 0 && (e[Qe.isEnteringWindow] = !0),
            (t.isOverWindow = !0),
            t.enterCount++);
        },
      },
      {
        type: `dragleave`,
        listener: function (e) {
          (t.enterCount--,
            t.isOverWindow &&
              t.enterCount === 0 &&
              ((e[Qe.isLeavingWindow] = !0), (t.isOverWindow = !1)));
        },
      },
    ],
    { capture: !0 },
  );
})();
function et(e) {
  return `nodeName` in e;
}
function tt(e) {
  return et(e) && e.ownerDocument !== document;
}
function nt(e) {
  var t = e.dragLeave,
    n = t.type,
    r = t.relatedTarget;
  return n === `dragleave`
    ? Ze()
      ? $e({ dragLeave: t })
      : r == null
        ? !0
        : Xe()
          ? tt(r)
          : r instanceof HTMLIFrameElement
    : !1;
}
function rt(e) {
  var t = e.onDragEnd;
  return [
    {
      type: `pointermove`,
      listener: (function () {
        var e = 0;
        return function () {
          if (e < 20) {
            e++;
            return;
          }
          t();
        };
      })(),
    },
    { type: `pointerdown`, listener: t },
  ];
}
function it(e) {
  return {
    altKey: e.altKey,
    button: e.button,
    buttons: e.buttons,
    ctrlKey: e.ctrlKey,
    metaKey: e.metaKey,
    shiftKey: e.shiftKey,
    clientX: e.clientX,
    clientY: e.clientY,
    pageX: e.pageX,
    pageY: e.pageY,
  };
}
var at = b(function (e) {
    return e();
  }),
  ot = (function () {
    var e = null;
    function t(t) {
      e = {
        frameId: requestAnimationFrame(function () {
          ((e = null), t());
        }),
        fn: t,
      };
    }
    function n() {
      e &&= (cancelAnimationFrame(e.frameId), e.fn(), null);
    }
    return { schedule: t, flush: n };
  })();
function st(e) {
  var t = e.source,
    n = e.initial,
    r = e.dispatchEvent,
    i = { dropTargets: [] };
  function a(e) {
    (r(e), (i = { dropTargets: e.payload.location.current.dropTargets }));
  }
  return {
    start: function (e) {
      var r = e.nativeSetDragImage,
        o = { current: n, previous: i, initial: n };
      (a({
        eventName: `onGenerateDragPreview`,
        payload: { source: t, location: o, nativeSetDragImage: r },
      }),
        ot.schedule(function () {
          a({ eventName: `onDragStart`, payload: { source: t, location: o } });
        }));
    },
    dragUpdate: function (e) {
      var r = e.current;
      (ot.flush(),
        at.cancel(),
        a({
          eventName: `onDropTargetChange`,
          payload: { source: t, location: { initial: n, previous: i, current: r } },
        }));
    },
    drag: function (e) {
      var r = e.current;
      at(function () {
        (ot.flush(),
          a({
            eventName: `onDrag`,
            payload: { source: t, location: { initial: n, previous: i, current: r } },
          }));
      });
    },
    drop: function (e) {
      var r = e.current,
        o = e.updatedSourcePayload;
      (ot.flush(),
        at.cancel(),
        a({
          eventName: `onDrop`,
          payload: { source: o ?? t, location: { current: r, previous: i, initial: n } },
        }));
    },
  };
}
var ct = { isActive: !1 };
function lt() {
  return !ct.isActive;
}
function ut(e) {
  return e.dataTransfer ? e.dataTransfer.setDragImage.bind(e.dataTransfer) : null;
}
function dt(e) {
  var t = e.current,
    n = e.next;
  if (t.length !== n.length) return !0;
  for (var r = 0; r < t.length; r++) if (t[r].element !== n[r].element) return !0;
  return !1;
}
function ft(e) {
  var t = e.event,
    n = e.dragType,
    r = e.getDropTargetsOver,
    i = e.dispatchEvent;
  if (!lt()) return;
  var a = mt({ event: t, dragType: n, getDropTargetsOver: r });
  ct.isActive = !0;
  var o = { current: a };
  pt({ event: t, current: a.dropTargets });
  var s = st({ source: n.payload, dispatchEvent: i, initial: a });
  function l(e) {
    var t = dt({ current: o.current.dropTargets, next: e.dropTargets });
    ((o.current = e), t && s.dragUpdate({ current: o.current }));
  }
  function u(e) {
    var t = it(e),
      i = r({
        target: Me(e.target) ? Ne({ x: t.clientX, y: t.clientY }) : e.target,
        input: t,
        source: n.payload,
        current: o.current.dropTargets,
      });
    (i.length && (e.preventDefault(), pt({ event: e, current: i })),
      l({ dropTargets: i, input: t }));
  }
  function d() {
    (o.current.dropTargets.length && l({ dropTargets: [], input: o.current.input }),
      s.drop({ current: o.current, updatedSourcePayload: null }),
      f());
  }
  function f() {
    ((ct.isActive = !1), p());
  }
  var p = (0, G.bindAll)(
    window,
    [
      {
        type: `dragover`,
        listener: function (e) {
          (u(e), s.drag({ current: o.current }));
        },
      },
      { type: `dragenter`, listener: u },
      {
        type: `dragleave`,
        listener: function (e) {
          nt({ dragLeave: e }) &&
            (l({ input: o.current.input, dropTargets: [] }), n.startedFrom === `external` && d());
        },
      },
      {
        type: `drop`,
        listener: function (e) {
          if (
            ((o.current = { dropTargets: o.current.dropTargets, input: it(e) }),
            !o.current.dropTargets.length)
          ) {
            d();
            return;
          }
          (e.preventDefault(),
            pt({ event: e, current: o.current.dropTargets }),
            s.drop({
              current: o.current,
              updatedSourcePayload: n.type === `external` ? n.getDropPayload(e) : null,
            }),
            f());
        },
      },
      {
        type: `dragend`,
        listener: function (e) {
          ((o.current = { dropTargets: o.current.dropTargets, input: it(e) }), d());
        },
      },
    ].concat(c(rt({ onDragEnd: d }))),
    { capture: !0 },
  );
  s.start({ nativeSetDragImage: ut(t) });
}
function pt(e) {
  var t = e.event,
    n = e.current[0]?.dropEffect;
  n != null && t.dataTransfer && (t.dataTransfer.dropEffect = n);
}
function mt(e) {
  var t = e.event,
    n = e.dragType,
    r = e.getDropTargetsOver,
    i = it(t);
  return n.startedFrom === `external`
    ? { input: i, dropTargets: [] }
    : { input: i, dropTargets: r({ input: i, source: n.payload, target: t.target, current: [] }) };
}
var ht = { canStart: lt, start: ft },
  gt = new Map();
function _t(e) {
  var t = e.typeKey,
    n = e.mount,
    r = gt.get(t);
  if (r) return (r.usageCount++, r);
  var i = { typeKey: t, unmount: n(), usageCount: 1 };
  return (gt.set(t, i), i);
}
function vt(e) {
  var t = _t(e);
  return function () {
    (t.usageCount--, !(t.usageCount > 0) && (t.unmount(), gt.delete(e.typeKey)));
  };
}
function yt(e, t) {
  var n = t.attribute,
    r = t.value;
  return (
    e.setAttribute(n, r),
    function () {
      return e.removeAttribute(n);
    }
  );
}
function bt(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function K(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? bt(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : bt(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
function xt(e, t) {
  var n = (typeof Symbol < `u` && e[Symbol.iterator]) || e[`@@iterator`];
  if (!n) {
    if (Array.isArray(e) || (n = St(e)) || (t && e && typeof e.length == `number`)) {
      n && (e = n);
      var r = 0,
        i = function () {};
      return {
        s: i,
        n: function () {
          return r >= e.length ? { done: !0 } : { done: !1, value: e[r++] };
        },
        e: function (e) {
          throw e;
        },
        f: i,
      };
    }
    throw TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
  }
  var a,
    o = !0,
    s = !1;
  return {
    s: function () {
      n = n.call(e);
    },
    n: function () {
      var e = n.next();
      return ((o = e.done), e);
    },
    e: function (e) {
      ((s = !0), (a = e));
    },
    f: function () {
      try {
        o || n.return == null || n.return();
      } finally {
        if (s) throw a;
      }
    },
  };
}
function St(e, t) {
  if (e) {
    if (typeof e == `string`) return Ct(e, t);
    var n = {}.toString.call(e).slice(8, -1);
    return (
      n === `Object` && e.constructor && (n = e.constructor.name),
      n === `Map` || n === `Set`
        ? Array.from(e)
        : n === `Arguments` || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)
          ? Ct(e, t)
          : void 0
    );
  }
}
function Ct(e, t) {
  (t == null || t > e.length) && (t = e.length);
  for (var n = 0, r = Array(t); n < t; n++) r[n] = e[n];
  return r;
}
function wt(e) {
  return e.slice(0).reverse();
}
function Tt(e) {
  var t = e.typeKey,
    n = e.defaultDropEffect,
    r = new WeakMap(),
    i = `data-drop-target-for-${t}`,
    a = `[${i}]`;
  function o(e) {
    return (
      r.set(e.element, e),
      function () {
        return r.delete(e.element);
      }
    );
  }
  function s(e) {
    return W(U(yt(e.element, { attribute: i, value: `true` }), o(e)));
  }
  function l(e) {
    var t = e.source,
      i = e.target,
      o = e.input,
      s = e.result,
      u = s === void 0 ? [] : s;
    if (i == null) return u;
    if (!(i instanceof Element))
      return i instanceof Node ? l({ source: t, target: i.parentElement, input: o, result: u }) : u;
    var d = i.closest(a);
    if (d == null) return u;
    var f = r.get(d);
    if (f == null) return u;
    var p = { input: o, source: t, element: f.element };
    if (f.canDrop && !f.canDrop(p))
      return l({ source: t, target: f.element.parentElement, input: o, result: u });
    var m = f.getData?.call(f, p) ?? {},
      h = f.getDropEffect?.call(f, p) ?? n,
      g = { data: m, element: f.element, dropEffect: h, isActiveDueToStickiness: !1 };
    return l({
      source: t,
      target: f.element.parentElement,
      input: o,
      result: [].concat(c(u), [g]),
    });
  }
  function u(e) {
    var t = e.eventName,
      n = e.payload,
      i = xt(n.location.current.dropTargets),
      a;
    try {
      for (i.s(); !(a = i.n()).done; ) {
        var o,
          s = a.value,
          c = r.get(s.element),
          l = K(K({}, n), {}, { self: s });
        c == null || (o = c[t]) == null || o.call(c, l);
      }
    } catch (e) {
      i.e(e);
    } finally {
      i.f();
    }
  }
  var d = {
    onGenerateDragPreview: u,
    onDrag: u,
    onDragStart: u,
    onDrop: u,
    onDropTargetChange: function (e) {
      var t = e.payload,
        n = new Set(
          t.location.current.dropTargets.map(function (e) {
            return e.element;
          }),
        ),
        i = new Set(),
        a = xt(t.location.previous.dropTargets),
        o;
      try {
        for (a.s(); !(o = a.n()).done; ) {
          var s,
            c = o.value;
          i.add(c.element);
          var l = r.get(c.element),
            u = n.has(c.element),
            d = K(K({}, t), {}, { self: c });
          if ((l == null || (s = l.onDropTargetChange) == null || s.call(l, d), !u)) {
            var f;
            l == null || (f = l.onDragLeave) == null || f.call(l, d);
          }
        }
      } catch (e) {
        a.e(e);
      } finally {
        a.f();
      }
      var p = xt(t.location.current.dropTargets),
        m;
      try {
        for (p.s(); !(m = p.n()).done; ) {
          var h,
            g,
            _ = m.value;
          if (!i.has(_.element)) {
            var v = K(K({}, t), {}, { self: _ }),
              y = r.get(_.element);
            (y == null || (h = y.onDropTargetChange) == null || h.call(y, v),
              y == null || (g = y.onDragEnter) == null || g.call(y, v));
          }
        }
      } catch (e) {
        p.e(e);
      } finally {
        p.f();
      }
    },
  };
  function f(e) {
    d[e.eventName](e);
  }
  function p(e) {
    var t = e.source,
      n = e.target,
      i = e.input,
      a = e.current,
      o = l({ source: t, target: n, input: i });
    if (o.length >= a.length) return o;
    for (var s = wt(a), c = wt(o), u = [], d = 0; d < s.length; d++) {
      var f,
        p = s[d],
        m = c[d];
      if (m != null) {
        u.push(m);
        continue;
      }
      var h = u[d - 1],
        g = s[d - 1];
      if (h?.element !== g?.element) break;
      var _ = r.get(p.element);
      if (!_) break;
      var v = { input: i, source: t, element: _.element };
      if ((_.canDrop && !_.canDrop(v)) || !((f = _.getIsSticky) != null && f.call(_, v))) break;
      u.push(K(K({}, p), {}, { isActiveDueToStickiness: !0 }));
    }
    return wt(u);
  }
  return { dropTargetForConsumers: s, getIsOver: p, dispatchEvent: f };
}
function Et(e, t) {
  var n = (typeof Symbol < `u` && e[Symbol.iterator]) || e[`@@iterator`];
  if (!n) {
    if (Array.isArray(e) || (n = Dt(e)) || (t && e && typeof e.length == `number`)) {
      n && (e = n);
      var r = 0,
        i = function () {};
      return {
        s: i,
        n: function () {
          return r >= e.length ? { done: !0 } : { done: !1, value: e[r++] };
        },
        e: function (e) {
          throw e;
        },
        f: i,
      };
    }
    throw TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
  }
  var a,
    o = !0,
    s = !1;
  return {
    s: function () {
      n = n.call(e);
    },
    n: function () {
      var e = n.next();
      return ((o = e.done), e);
    },
    e: function (e) {
      ((s = !0), (a = e));
    },
    f: function () {
      try {
        o || n.return == null || n.return();
      } finally {
        if (s) throw a;
      }
    },
  };
}
function Dt(e, t) {
  if (e) {
    if (typeof e == `string`) return Ot(e, t);
    var n = {}.toString.call(e).slice(8, -1);
    return (
      n === `Object` && e.constructor && (n = e.constructor.name),
      n === `Map` || n === `Set`
        ? Array.from(e)
        : n === `Arguments` || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)
          ? Ot(e, t)
          : void 0
    );
  }
}
function Ot(e, t) {
  (t == null || t > e.length) && (t = e.length);
  for (var n = 0, r = Array(t); n < t; n++) r[n] = e[n];
  return r;
}
function kt(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function At(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? kt(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : kt(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
function jt() {
  var e = new Set(),
    t = null;
  function n(e) {
    t && (!e.canMonitor || e.canMonitor(t.canMonitorArgs)) && t.active.add(e);
  }
  function r(r) {
    var i = At({}, r);
    (e.add(i), n(i));
    function a() {
      (e.delete(i), t && t.active.delete(i));
    }
    return W(a);
  }
  function i(r) {
    var i = r.eventName,
      a = r.payload;
    if (i === `onGenerateDragPreview`) {
      t = { canMonitorArgs: { initial: a.location.initial, source: a.source }, active: new Set() };
      var o = Et(e),
        s;
      try {
        for (o.s(); !(s = o.n()).done; ) {
          var c = s.value;
          n(c);
        }
      } catch (e) {
        o.e(e);
      } finally {
        o.f();
      }
    }
    if (t) {
      for (var l = Array.from(t.active), u = 0, d = l; u < d.length; u++) {
        var f = d[u];
        if (t.active.has(f)) {
          var p;
          (p = f[i]) == null || p.call(f, a);
        }
      }
      i === `onDrop` && (t.active.clear(), (t = null));
    }
  }
  return { dispatchEvent: i, monitorForConsumers: r };
}
function Mt(e) {
  var t = e.typeKey,
    n = e.mount,
    r = e.dispatchEventToSource,
    i = e.onPostDispatch,
    a = e.defaultDropEffect,
    o = jt(),
    s = Tt({ typeKey: t, defaultDropEffect: a });
  function c(e) {
    (r?.(e), s.dispatchEvent(e), o.dispatchEvent(e), i?.(e));
  }
  function l(e) {
    var t = e.event,
      n = e.dragType;
    ht.start({ event: t, dragType: n, getDropTargetsOver: s.getIsOver, dispatchEvent: c });
  }
  function u() {
    function e() {
      return n({ canStart: ht.canStart, start: l });
    }
    return vt({ typeKey: t, mount: e });
  }
  return { registerUsage: u, dropTarget: s.dropTargetForConsumers, monitor: o.monitorForConsumers };
}
var Nt = W(function () {
    return navigator.userAgent.toLocaleLowerCase().includes(`android`);
  }),
  Pt = `pdnd:android-fallback`,
  Ft = `text/plain`,
  It = `application/vnd.pdnd`,
  Lt = new WeakMap();
function Rt(e) {
  return (
    Lt.set(e.element, e),
    function () {
      Lt.delete(e.element);
    }
  );
}
var zt = Ye(),
  Bt = Mt({
    typeKey: `element`,
    defaultDropEffect: `move`,
    mount: function (e) {
      return U(
        zt.bindEvents(),
        (0, G.bind)(document, {
          type: `dragstart`,
          listener: function (t) {
            if (e.canStart(t) && !t.defaultPrevented && t.dataTransfer) {
              var n = t.target;
              if (n instanceof HTMLElement) {
                var r = Lt.get(n);
                if (r) {
                  var i = it(t),
                    a = { element: r.element, dragHandle: r.dragHandle ?? null, input: i };
                  if (r.canDrag && !r.canDrag(a)) {
                    t.preventDefault();
                    return;
                  }
                  if (r.dragHandle) {
                    var o = Ne({ x: i.clientX, y: i.clientY });
                    if (!r.dragHandle.contains(o)) {
                      t.preventDefault();
                      return;
                    }
                  }
                  var s = r.getInitialDataForExternal?.call(r, a) ?? null;
                  if (s)
                    for (var c = 0, l = Object.entries(s); c < l.length; c++) {
                      var u = h(l[c], 2),
                        d = u[0],
                        f = u[1];
                      t.dataTransfer.setData(d, f ?? ``);
                    }
                  (Nt() &&
                    !t.dataTransfer.types.includes(`text/plain`) &&
                    !t.dataTransfer.types.includes(`text/uri-list`) &&
                    t.dataTransfer.setData(Ft, Pt),
                    t.dataTransfer.setData(It, ``));
                  var p = {
                    type: `element`,
                    payload: {
                      element: r.element,
                      dragHandle: r.dragHandle ?? null,
                      data: r.getInitialData?.call(r, a) ?? {},
                    },
                    startedFrom: `internal`,
                  };
                  e.start({ event: t, dragType: p });
                }
              }
            }
          },
        }),
      );
    },
    dispatchEventToSource: function (e) {
      var t,
        n,
        r = e.eventName,
        i = e.payload;
      (t = Lt.get(i.source.element)) == null || (n = t[r]) == null || n.call(t, i);
    },
    onPostDispatch: zt.getOnPostDispatch(),
  }),
  Vt = Bt.dropTarget,
  Ht = Bt.monitor;
function Ut(e) {
  return W(U(Bt.registerUsage(), Rt(e), yt(e.element, { attribute: `draggable`, value: `true` })));
}
function Wt(e, t, n) {
  return ((t = u(t)), w(e, Gt() ? Reflect.construct(t, n || [], u(e).constructor) : t.apply(e, n)));
}
function Gt() {
  try {
    var e = !Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {}));
  } catch {}
  return (Gt = function () {
    return !!e;
  })();
}
function Kt(e) {
  var t = e.children,
    n = e.dragController,
    r = (0, M.useRef)(!1),
    i = (0, M.useCallback)(
      function (e) {
        var t = n.getDragState();
        t.isDragging &&
          (t.mode === `FLUID` && ue(),
          t.mode === `SNAP` && n.stopDrag({ reason: `CANCEL` }),
          e.error instanceof A && e.preventDefault());
      },
      [n],
    );
  return (
    (0, M.useEffect)(
      function () {
        return U(
          Ht({
            onDragStart: function () {
              r.current = !0;
            },
            onDrop: function () {
              r.current = !1;
            },
          }),
          (0, G.bind)(window, { type: `error`, listener: i }),
        );
      },
      [i],
    ),
    t
  );
}
var qt = (function (e) {
  function t() {
    return (l(this, t), Wt(this, t, arguments));
  }
  return (
    T(t, e),
    C(
      t,
      [
        {
          key: `componentDidCatch`,
          value: function (e) {
            if (!(e instanceof A)) throw e;
          },
        },
        {
          key: `render`,
          value: function () {
            return M.createElement(
              Kt,
              { contextId: this.props.contextId, dragController: this.props.dragController },
              this.props.children,
            );
          },
        },
      ],
      [{ key: `getDerivedStateFromError`, value: function () {} }],
    )
  );
})(M.Component);
function Jt(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function Yt(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? Jt(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : Jt(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
function Xt(e) {
  var t = e.start,
    n = e.target;
  if (n === null) return null;
  var r = t.droppableId === n.droppableId,
    i = n.index > t.index;
  return r && i ? Yt(Yt({}, n), {}, { index: n.index - 1 }) : Yt({}, n);
}
function Zt(e) {
  return `rbd-lift-instruction-${e}`;
}
function Qt(e) {
  var t = e.contextId,
    n = e.text;
  (0, M.useEffect)(
    function () {
      var e = Zt(t),
        r = document.createElement(`div`);
      return (
        (r.id = e),
        (r.textContent = n),
        Object.assign(r.style, { display: `none` }),
        document.body.appendChild(r),
        function () {
          r.remove();
        }
      );
    },
    [t, n],
  );
}
function $t(e) {
  var t = getComputedStyle(e),
    n = t.overflowX,
    r = t.overflowY;
  if (n === `scroll` || n === `auto` || r === `scroll` || r === `auto`) return e;
  var i = e.parentElement;
  return i === null ? null : $t(i);
}
function en(e) {
  var t = e.contextId,
    n = e.type;
  return oe(
    { attribute: L.droppable.contextId, value: t },
    { attribute: R.droppable.type, value: n },
  );
}
function tn(e) {
  var t = e.droppableId,
    n = e.type,
    r = e.isMovingForward,
    i = e.contextId,
    a = e.droppableRegistry,
    o = en({ contextId: i, type: n }),
    s = o.findIndex(function (e) {
      return z(e, L.droppable.id) === t;
    }),
    c = o
      .filter(function (e, t) {
        return r ? t > s : t < s;
      })
      .filter(function (e) {
        var t = z(e, L.droppable.id),
          n = a.getEntry({ droppableId: t });
        return n && !n.isDropDisabled;
      });
  return (r ? c.at(0) : c.at(-1)) ?? null;
}
function nn(e, t) {
  return t
    ? B(
        { attribute: L.draggable.contextId, value: e },
        { attribute: R.draggable.droppableId, value: t.droppableId },
        { attribute: R.draggable.index, value: String(t.index) },
      )
    : null;
}
function rn(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function q(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? rn(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : rn(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
function an(e) {
  var t = $t(e);
  t && t.scrollTo(0, 0);
}
var J = {
  mainAxis: {
    prev: function (e, t) {
      var n = t.dragController;
      e.preventDefault();
      var r = n.getDragState();
      j(r.isDragging);
      var i = r.sourceLocation,
        a = r.targetLocation;
      if (a && a.index !== 0) {
        var o = q(q({}, a), {}, { index: a.index - 1 });
        (Oe(i, Xt({ start: i, target: o })) && (o.index = a.index - 2),
          n.updateDrag({ targetLocation: o }));
      }
    },
    next: function (e, t) {
      var n = t.dragController,
        r = t.contextId;
      e.preventDefault();
      var i = n.getDragState();
      j(i.isDragging);
      var a = i.sourceLocation,
        o = i.targetLocation;
      if (o) {
        var s = nn(r, o);
        if (!(!Oe(a, o) && !s)) {
          var c = q(q({}, o), {}, { index: o.index + 1 });
          (Oe(a, Xt({ start: a, target: c })) && (c.index = o.index + 2),
            n.updateDrag({ targetLocation: c }));
        }
      }
    },
  },
  crossAxis: {
    prev: function (e, t) {
      var n = t.dragController,
        r = t.droppableRegistry,
        i = t.contextId;
      e.preventDefault();
      var a = n.getDragState();
      j(a.isDragging);
      var o = a.targetLocation,
        s = a.type;
      if (o) {
        var c = tn({
          droppableId: o.droppableId,
          type: s,
          isMovingForward: !1,
          contextId: i,
          droppableRegistry: r,
        });
        if (c) {
          an(c);
          var l = { droppableId: z(c, L.droppable.id), index: 0 };
          n.updateDrag({ targetLocation: l });
        }
      }
    },
    next: function (e, t) {
      var n = t.dragController,
        r = t.droppableRegistry,
        i = t.contextId;
      e.preventDefault();
      var a = n.getDragState();
      j(a.isDragging);
      var o = a.targetLocation,
        s = a.type;
      if (o) {
        var c = tn({
          droppableId: o.droppableId,
          type: s,
          isMovingForward: !0,
          contextId: i,
          droppableRegistry: r,
        });
        if (c) {
          an(c);
          var l = { droppableId: z(c, L.droppable.id), index: 0 };
          n.updateDrag({ targetLocation: l });
        }
      }
    },
  },
};
function on(e) {
  e.preventDefault();
}
var sn = { PageUp: on, PageDown: on, Home: on, End: on, Enter: on, Tab: on },
  cn = {
    vertical: q(
      q({}, sn),
      {},
      {
        ArrowUp: J.mainAxis.prev,
        ArrowDown: J.mainAxis.next,
        ArrowLeft: J.crossAxis.prev,
        ArrowRight: J.crossAxis.next,
      },
    ),
    horizontal: q(
      q({}, sn),
      {},
      {
        ArrowUp: J.crossAxis.prev,
        ArrowDown: J.crossAxis.next,
        ArrowLeft: J.mainAxis.prev,
        ArrowRight: J.mainAxis.next,
      },
    ),
  };
function ln(e) {
  var t = e.dragController,
    n = e.droppableRegistry,
    r = e.contextId,
    i = e.setKeyboardCleanupFn;
  return {
    startKeyboardDrag: (0, M.useCallback)(
      function (e) {
        var a = e.event,
          o = e.draggableId,
          s = e.type,
          l = e.getSourceLocation,
          u = e.sourceElement;
        t.startDrag({
          draggableId: o,
          type: s,
          getSourceLocation: l,
          sourceElement: u,
          mode: `SNAP`,
        });
        var d = l(),
          f = z(V({ attribute: L.droppable.id, value: d.droppableId }), R.droppable.direction);
        j(f === `vertical` || f === `horizontal`);
        function p() {
          t.stopDrag({ reason: `CANCEL` });
        }
        var m = [
          `mousedown`,
          `mouseup`,
          `click`,
          `touchstart`,
          `resize`,
          `wheel`,
          `visibilitychange`,
        ].map(function (e) {
          return { type: e, listener: p };
        });
        i(
          (0, G.bindAll)(
            window,
            [
              {
                type: `keydown`,
                listener: function (e) {
                  var i, o;
                  if (e !== a && t.getDragState().isDragging) {
                    if (e.key === ` `) {
                      (e.preventDefault(), t.stopDrag({ reason: `DROP` }));
                      return;
                    }
                    if (e.key === `Escape`) {
                      (e.preventDefault(), t.stopDrag({ reason: `CANCEL` }));
                      return;
                    }
                    (i = (o = cn[f])[e.key]) == null ||
                      i.call(o, e, { dragController: t, droppableRegistry: n, contextId: r });
                  }
                },
              },
            ].concat(c(m)),
          ),
        );
      },
      [r, t, n, i],
    ),
  };
}
var un = function (e) {
    return function (t) {
      return e === t;
    };
  },
  dn = un(`scroll`),
  fn = un(`auto`),
  pn = function (e, t) {
    return t(e.overflowX) || t(e.overflowY);
  },
  mn = function (e) {
    var t = window.getComputedStyle(e),
      n = { overflowX: t.overflowX, overflowY: t.overflowY };
    return pn(n, dn) || pn(n, fn);
  },
  hn = function (e) {
    return !e || e === document.body || e === document.documentElement
      ? null
      : mn(e)
        ? e
        : hn(e.parentElement);
  },
  gn = function (e) {
    var t = e.top,
      n = e.right,
      r = e.bottom,
      i = e.left;
    return {
      top: t,
      right: n,
      bottom: r,
      left: i,
      width: n - i,
      height: r - t,
      x: i,
      y: t,
      center: { x: (n + i) / 2, y: (r + t) / 2 },
    };
  },
  _n = function (e) {
    var t = e.closestScrollable,
      n = t.getBoundingClientRect(),
      r = { x: t.scrollLeft, y: t.scrollTop };
    return {
      container: gn(n),
      scroll: {
        current: r,
        max: { x: t.scrollWidth - t.clientWidth, y: t.scrollHeight - t.clientHeight },
      },
    };
  },
  vn = { x: 0, y: 0 },
  yn = function (e) {
    return function (t) {
      return { x: e(t.x), y: e(t.y) };
    };
  },
  bn = function (e, t) {
    return e.x === t.x && e.y === t.y;
  },
  xn = function (e, t) {
    return { x: e.x + t.x, y: e.y + t.y };
  },
  Sn = function (e, t) {
    return { x: e.x - t.x, y: e.y - t.y };
  },
  Cn = yn(function (e) {
    return e === 0 ? 0 : e > 0 ? 1 : -1;
  }),
  wn = (function () {
    var e = function (e, t) {
      return e < 0 ? e : e > t ? e - t : 0;
    };
    return function (t) {
      var n = t.current,
        r = t.max,
        i = t.change,
        a = xn(n, i),
        o = { x: e(a.x, r.x), y: e(a.y, r.y) };
      return bn(o, vn) ? null : o;
    };
  })(),
  Tn = function (e) {
    var t = e.max,
      n = e.current,
      r = e.change,
      i = { x: Math.max(n.x, t.x), y: Math.max(n.y, t.y) },
      a = Cn(r),
      o = wn({ max: i, current: n, change: a });
    return !o || (a.x !== 0 && o.x === 0) || (a.y !== 0 && o.y === 0);
  },
  En = function (e, t) {
    return Tn({ current: e.scroll.current, max: e.scroll.max, change: t });
  },
  Dn = function (e, t) {
    return Tn({ current: e.scroll.current, max: e.scroll.max, change: t });
  },
  On = {
    direction: `vertical`,
    start: `top`,
    end: `bottom`,
    size: `height`,
    scrollAxis: `scrollTop`,
  },
  kn = {
    direction: `horizontal`,
    start: `left`,
    end: `right`,
    size: `width`,
    scrollAxis: `scrollLeft`,
  };
(kn.direction, On.direction);
var Y = {
    startFromPercentage: 0.25,
    maxScrollAtPercentage: 0.05,
    maxPixelScroll: 28,
    ease: function (e) {
      return e ** 2;
    },
    durationDampening: { stopDampeningAt: 1200, accelerateAt: 360 },
  },
  An = function (e, t) {
    return {
      startScrollingFrom: e[t.size] * Y.startFromPercentage,
      maxScrollValueAt: e[t.size] * Y.maxScrollAtPercentage,
    };
  },
  jn = function (e) {
    var t = e.startOfRange,
      n = e.endOfRange,
      r = e.current,
      i = n - t;
    return i === 0 ? 0 : (r - t) / i;
  },
  Mn = Y.durationDampening.accelerateAt,
  Nn = Y.durationDampening.stopDampeningAt,
  Pn = function (e, t) {
    var n = t,
      r = Nn,
      i = Date.now() - n;
    if (i >= Nn) return e;
    if (i < Mn) return 1;
    var a = jn({ startOfRange: Mn, endOfRange: r, current: i }),
      o = e * Y.ease(a);
    return Math.ceil(o);
  },
  Fn = function (e, t) {
    if (e > t.startScrollingFrom) return 0;
    if (e <= t.maxScrollValueAt) return Y.maxPixelScroll;
    if (e === t.startScrollingFrom) return 1;
    var n =
        1 - jn({ startOfRange: t.maxScrollValueAt, endOfRange: t.startScrollingFrom, current: e }),
      r = Y.maxPixelScroll * Y.ease(n);
    return Math.ceil(r);
  },
  In = function (e) {
    var t = e.distanceToEdge,
      n = e.thresholds,
      r = e.dragStartTime,
      i = e.shouldUseTimeDampening,
      a = Fn(t, n);
    return a === 0 ? 0 : i ? Math.max(Pn(a, r), 1) : a;
  },
  Ln = function (e) {
    var t = e.container,
      n = e.distanceToEdges,
      r = e.dragStartTime,
      i = e.axis,
      a = e.shouldUseTimeDampening,
      o = An(t, i);
    return n[i.end] < n[i.start]
      ? In({ distanceToEdge: n[i.end], thresholds: o, dragStartTime: r, shouldUseTimeDampening: a })
      : -1 *
          In({
            distanceToEdge: n[i.start],
            thresholds: o,
            dragStartTime: r,
            shouldUseTimeDampening: a,
          });
  },
  Rn = yn(function (e) {
    return e === 0 ? 0 : e;
  }),
  zn = function (e) {
    var t = e.dragStartTime,
      n = e.container,
      r = e.center,
      i = e.shouldUseTimeDampening,
      a = { top: r.y - n.top, right: n.right - r.x, bottom: n.bottom - r.y, left: r.x - n.left },
      o = Ln({
        container: n,
        distanceToEdges: a,
        dragStartTime: t,
        axis: On,
        shouldUseTimeDampening: i,
      }),
      s = Rn({
        x: Ln({
          container: n,
          distanceToEdges: a,
          dragStartTime: t,
          axis: kn,
          shouldUseTimeDampening: i,
        }),
        y: o,
      });
    return bn(s, vn) ? null : s;
  },
  Bn = function (e) {
    var t = e.scrollable,
      n = e.center,
      r = e.dragStartTime,
      i = e.shouldUseTimeDampening,
      a = zn({ dragStartTime: r, container: t.container, center: n, shouldUseTimeDampening: i });
    return a && Dn(t, a) ? a : null;
  },
  Vn = function (e) {
    var t = e.viewport,
      n = e.center,
      r = e.dragStartTime,
      i = e.shouldUseTimeDampening,
      a = zn({ dragStartTime: r, container: t.container, center: n, shouldUseTimeDampening: i });
    return a && En(t, a) ? a : null;
  },
  Hn = function (e) {
    var t = e.scrollHeight,
      n = e.scrollWidth,
      r = e.height,
      i = e.width,
      a = Sn({ x: n, y: t }, { x: i, y: r });
    return { x: Math.max(0, a.x), y: Math.max(0, a.y) };
  },
  Un = function () {
    var e = document.documentElement;
    return Hn({
      scrollHeight: e.scrollHeight,
      scrollWidth: e.scrollWidth,
      width: e.clientWidth,
      height: e.clientHeight,
    });
  },
  Wn = function () {
    return { x: window.pageXOffset, y: window.pageYOffset };
  },
  Gn = function () {
    var e = Wn(),
      t = Un(),
      n = e.y,
      r = e.x,
      i = document.documentElement,
      a = i.clientWidth,
      o = i.clientHeight;
    return {
      container: gn({ top: n, left: r, right: r + a, bottom: n + o }),
      scroll: { current: e, max: t },
    };
  },
  Kn = function (e) {
    var t = e.input,
      n = e.dragStartTime,
      r = e.shouldUseTimeDampening,
      i = e.scrollElement,
      a = e.scrollWindow,
      o = e.behavior,
      s = function () {
        var e = Gn(),
          i = Vn({
            dragStartTime: n,
            viewport: e,
            center: { x: t.clientX + e.scroll.current.x, y: t.clientY + e.scroll.current.y },
            shouldUseTimeDampening: r,
          });
        return i ? (a(i), !0) : !1;
      },
      c = function () {
        var e = hn(Ne({ x: t.clientX, y: t.clientY }));
        if (!e) return !1;
        var a = Bn({
          dragStartTime: n,
          scrollable: _n({ closestScrollable: e }),
          center: { x: t.clientX, y: t.clientY },
          shouldUseTimeDampening: r,
        });
        return a ? (i(e, a), !0) : !1;
      };
    (o === `container-only` && c(),
      o === `window-only` && s(),
      o === `container-then-window` && (c() || s()),
      o === `window-then-container` && (s() || c()));
  },
  qn = function (e, t) {
    e.scrollBy(t.x, t.y);
  },
  Jn = function (e) {
    window.scrollBy(e.x, e.y);
  },
  Yn = (function () {
    var e = null;
    function t(t) {
      e != null &&
        Kn({
          input: e.latestInput,
          dragStartTime: e.dragStartTime,
          shouldUseTimeDampening: e.shouldUseTimeDampening,
          behavior: e.behavior,
          scrollElement: t ?? qn,
          scrollWindow: t ?? Jn,
        });
    }
    function n() {
      e &&
        (e.loopFrameId = requestAnimationFrame(function () {
          (t(), n());
        }));
    }
    var r = function (r) {
      var i = r.input,
        a = r.behavior,
        o = a === void 0 ? `window-then-container` : a;
      ((e = {
        dragStartTime: Date.now(),
        latestInput: i,
        loopFrameId: null,
        shouldUseTimeDampening: !1,
        behavior: o,
      }),
        t(function () {
          e && (e.shouldUseTimeDampening = !0);
        }),
        n());
    };
    function i(t) {
      var n = t.input;
      e && (e.latestInput = n);
    }
    return {
      start: r,
      updateInput: i,
      stop: function () {
        e &&= (e.loopFrameId && cancelAnimationFrame(e.loopFrameId), null);
      },
    };
  })();
function Xn(e) {
  e.defaultPrevented ||
    (e.dataTransfer && (e.dataTransfer.dropEffect = `move`), e.preventDefault());
}
var Zn = null;
function Qn() {
  ($n(),
    (Zn = (0, G.bindAll)(
      window,
      [
        { type: `dragover`, listener: Xn },
        { type: `dragenter`, listener: Xn },
        {
          type: `drop`,
          listener: function (e) {
            (e.preventDefault(), $n());
          },
        },
        { type: `dragend`, listener: $n },
      ].concat(c(rt({ onDragEnd: $n }))),
      { capture: !1 },
    )));
}
function $n() {
  var e;
  ((e = Zn) == null || e(), (Zn = null));
}
function er() {
  if (window.event?.type === `drop`) {
    var e;
    (e = window.event) == null || e.preventDefault();
  }
  $n();
}
var tr = { start: Qn, stop: er };
function nr(e) {
  var t = e.dragController,
    n = e.contextId,
    r = (0, M.useCallback)(
      function (e) {
        t.updateDrag({ targetLocation: De(e.current) });
      },
      [t],
    );
  (0, M.useEffect)(
    function () {
      return Ht({
        canMonitor: function (e) {
          var t = e.initial,
            r = e.source;
          if (!H(r.data) || r.data.contextId !== n) return !1;
          var i = t.dropTargets.find(function (e) {
            return be(e.data);
          });
          return i ? i.data.contextId === n : !0;
        },
        onDragStart: function (e) {
          var n = e.location,
            r = e.source;
          (Yn.start({ input: n.current.input }), tr.start());
          var i = r.data;
          j(H(i));
          var a = i.draggableId,
            o = i.droppableId,
            s = i.getIndex,
            c = i.type;
          t.startDrag({
            draggableId: a,
            type: c,
            getSourceLocation: function () {
              return { droppableId: o, index: s() };
            },
            sourceElement: r.element,
            mode: `FLUID`,
          });
        },
        onDrag: function (e) {
          var t = e.location;
          (Yn.updateInput({ input: t.current.input }), r(t));
        },
        onDropTargetChange: function (e) {
          var t = e.location;
          r(t);
        },
        onDrop: function () {
          (Yn.stop(), tr.stop(), t.stopDrag({ reason: `DROP` }));
        },
      });
    },
    [t, n, r],
  );
}
var rr = typeof window < `u` ? M.useLayoutEffect : M.useEffect,
  ir = `data-rbd-style-context-id`;
function ar(e) {
  var t = e.selector,
    n = e.styles;
  return `${t} { ${Object.entries(n)
    .map(function (e) {
      var t = h(e, 2);
      return `${t[0]}: ${t[1]};`;
    })
    .join(` `)} }`;
}
function or(e) {
  return ar({
    selector: `[${L.dragHandle.contextId}="${e}"]`,
    styles: { cursor: `grab`, "-webkit-touch-callout": `none` },
  });
}
function sr(e) {
  var t = e.contextId,
    n = e.nonce,
    r = document.createElement(`style`);
  return (n && r.setAttribute(`nonce`, n), r.setAttribute(ir, t), document.head.appendChild(r), r);
}
function cr(e) {
  var t = e.contextId,
    n = e.nonce,
    r = sr({ contextId: t, nonce: n });
  return (
    (r.textContent = or(t)),
    function () {
      r.remove();
    }
  );
}
function lr(e) {
  var t = e.contextId,
    n = e.nonce;
  rr(
    function () {
      return cr({ contextId: t, nonce: n });
    },
    [t, n],
  );
}
var X = e(t()),
  ur = (function () {
    return typeof X.unstable_batchedUpdates == `function` &&
      (X.version === void 0 || X.version.startsWith(`16`))
      ? X.unstable_batchedUpdates
      : function (e) {
          return e();
        };
  })();
function dr(e) {
  ur(e);
}
function fr(e, t) {
  var n = (typeof Symbol < `u` && e[Symbol.iterator]) || e[`@@iterator`];
  if (!n) {
    if (Array.isArray(e) || (n = pr(e)) || (t && e && typeof e.length == `number`)) {
      n && (e = n);
      var r = 0,
        i = function () {};
      return {
        s: i,
        n: function () {
          return r >= e.length ? { done: !0 } : { done: !1, value: e[r++] };
        },
        e: function (e) {
          throw e;
        },
        f: i,
      };
    }
    throw TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
  }
  var a,
    o = !0,
    s = !1;
  return {
    s: function () {
      n = n.call(e);
    },
    n: function () {
      var e = n.next();
      return ((o = e.done), e);
    },
    e: function (e) {
      ((s = !0), (a = e));
    },
    f: function () {
      try {
        o || n.return == null || n.return();
      } finally {
        if (s) throw a;
      }
    },
  };
}
function pr(e, t) {
  if (e) {
    if (typeof e == `string`) return mr(e, t);
    var n = {}.toString.call(e).slice(8, -1);
    return (
      n === `Object` && e.constructor && (n = e.constructor.name),
      n === `Map` || n === `Set`
        ? Array.from(e)
        : n === `Arguments` || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)
          ? mr(e, t)
          : void 0
    );
  }
}
function mr(e, t) {
  (t == null || t > e.length) && (t = e.length);
  for (var n = 0, r = Array(t); n < t; n++) r[n] = e[n];
  return r;
}
function hr() {
  return {
    onPendingDragStart: [],
    onPrePendingDragUpdate: [],
    onPendingDragUpdate: [],
    onBeforeDragEnd: [],
  };
}
function gr() {
  var e = hr();
  return {
    addResponder: function (t, n) {
      return (
        e[t].push(n),
        function () {
          e[t] = e[t].filter(function (e) {
            return e !== n;
          });
        }
      );
    },
    dispatch: function (t, n) {
      dr(function () {
        var r = fr(e[t]),
          i;
        try {
          for (r.s(); !(i = r.n()).done; ) {
            var a = i.value;
            a(n);
          }
        } catch (e) {
          r.e(e);
        } finally {
          r.f();
        }
      });
    },
  };
}
function _r() {
  return h((0, M.useState)(gr), 1)[0];
}
var vr = (0, M.createContext)(null);
function yr(e) {
  var t = e.children,
    n = e.lifecycle,
    r = (0, M.useCallback)(
      function (e) {
        for (var t = [], r = 0, i = Object.entries(e); r < i.length; r++) {
          var a = i[r],
            o = h(a, 2),
            s = o[0],
            c = o[1];
          t.push(n.addResponder(s, c));
        }
        return U.apply(void 0, t);
      },
      [n],
    );
  return M.createElement(vr.Provider, { value: r }, t);
}
function br() {
  var e = (0, M.useContext)(vr);
  return (j(e !== null, `useLifecycle() should only be called inside of a <DragDropContext />`), e);
}
var xr = null,
  Z = `1px`,
  Sr = {
    width: Z,
    height: Z,
    padding: `0`,
    position: `absolute`,
    border: `0`,
    clip: `rect(${Z}, ${Z}, ${Z}, ${Z})`,
    overflow: `hidden`,
    whiteSpace: `nowrap`,
    marginTop: `-${Z}`,
    pointerEvents: `none`,
  };
function Cr() {
  var e = document.createElement(`div`);
  return (e.setAttribute(`role`, `alert`), Object.assign(e.style, Sr), document.body.append(e), e);
}
function wr() {
  return (xr === null && (xr = Cr()), xr);
}
function Tr(e) {
  var t = wr();
  t.textContent = e;
}
function Er(e) {
  return e.index + 1;
}
var Dr = {
  onDragStart: function (e) {
    var t = e.source;
    return `You have lifted an item in position ${Er(t)}.`;
  },
  onDragUpdate: function (e) {
    var t = e.source,
      n = e.destination;
    if (!n) return `You are currently not dragging over a droppable area.`;
    var r = Er(t),
      i = Er(n);
    return t.droppableId === n.droppableId
      ? `You have moved the item from position ${r} to position ${i}.`
      : `You have moved the item from position ${r} in list ${t.droppableId} to list ${n.droppableId} in position ${i}.`;
  },
  onDragEnd: function (e) {
    var t = e.source,
      n = e.destination,
      r = e.reason,
      i = Er(t);
    if (r === `CANCEL`)
      return `Movement cancelled. The item has returned to its starting position of ${i}.`;
    if (!n)
      return `The item has been dropped while not over a droppable location. The item has returned to its starting position of ${i}.`;
    var a = Er(n);
    return t.droppableId === n.droppableId
      ? `You have dropped the item. It has moved from position ${i} to ${a}.`
      : `You have dropped the item. It has moved from position ${i} in list ${t.droppableId} to position ${a} in list ${n.droppableId}.`;
  },
};
function Or(e, t) {
  return Dr[e](t);
}
function kr(e, t) {
  var n = null,
    r = {
      announce: function (e) {
        n = e;
      },
    };
  function i() {
    return n ?? Or(e, t);
  }
  return { provided: r, getMessage: i };
}
var Ar = `
  Press space bar to start a drag.
  When dragging you can use the arrow keys to move the item around and escape to cancel.
  Some screen readers may require you to be in focus mode or to use your pass through key
`,
  jr = { status: `idle` };
function Mr() {
  var e = jr,
    t = function (t) {
      (e.status === `idle` && (e = { status: `pending`, timeoutId: setTimeout(n, 0), items: [] }),
        e.items.push(t));
    },
    n = function () {
      if (e.status !== `idle`) {
        clearTimeout(e.timeoutId);
        var t = Array.from(e.items);
        ((e = jr),
          dr(function () {
            t.forEach(function (e) {
              return e();
            });
          }));
      }
    };
  return { schedule: t, flush: n };
}
function Nr() {
  return h((0, M.useState)(Mr), 1)[0];
}
var Pr = 0;
function Fr() {
  return `${Pr++}`;
}
function Ir(e) {
  var t = le(e);
  return { top: t.offsetTop, left: t.offsetLeft };
}
function Lr(e) {
  var t = e.children,
    n = e.dragHandleUsageInstructions,
    r = n === void 0 ? Ar : n,
    i = e.nonce,
    a = e.onBeforeCapture,
    o = e.onBeforeDragStart,
    s = e.onDragStart,
    c = e.onDragUpdate,
    l = e.onDragEnd,
    u = h((0, M.useState)(Fr), 1)[0];
  Qt({ contextId: u, text: r });
  var d = _r(),
    f = Nr(),
    p = f.schedule,
    m = f.flush,
    g = (0, M.useRef)({ isDragging: !1 }),
    _ = (0, M.useCallback)(function () {
      return g.current;
    }, []),
    v = Ae(),
    y = (0, M.useCallback)(
      function (e) {
        for (
          var t = e.droppableId, n = v.getEntry({ droppableId: t });
          n !== null && n.isDropDisabled;
        ) {
          var r = n.parentDroppableId;
          n = r ? v.getEntry({ droppableId: r }) : null;
        }
        return n === null ? null : { droppableId: n.droppableId, index: 0 };
      },
      [v],
    );
  (0, M.useEffect)(
    function () {
      return function () {
        _().isDragging && ue();
      };
    },
    [_],
  );
  var b = (0, M.useCallback)(
      function (e) {
        var t = e.targetLocation,
          n = e.isImmediate,
          r = n === void 0 ? !1 : n;
        if (!g.current.isDragging) return;
        var i = g.current,
          a = i.prevDestination,
          o = i.draggableId,
          s = i.type,
          l = i.sourceLocation,
          u = Xt({ start: l, target: t });
        if (Oe(a, u)) return;
        Object.assign(g.current, { prevDestination: u, sourceLocation: l, targetLocation: t });
        var f = {
            mode: g.current.mode,
            draggableId: o,
            type: s,
            source: l,
            destination: u,
            combine: null,
          },
          m = t ? v.getEntry({ droppableId: t.droppableId }) : null;
        (d.dispatch(`onPrePendingDragUpdate`, { update: f, targetLocation: t }),
          d.dispatch(`onPendingDragUpdate`, { update: f, targetLocation: t, droppable: m }));
        function h() {
          var e = kr(`onDragUpdate`, f),
            t = e.provided,
            n = e.getMessage;
          (c?.(f, t), Tr(n()));
        }
        r ? h() : p(h);
      },
      [v, d, c, p],
    ),
    x = (0, M.useCallback)(
      function (e) {
        var t = e.draggableId,
          n = e.type,
          r = e.getSourceLocation,
          i = e.sourceElement,
          c = e.mode;
        if (!g.current.isDragging) {
          a?.({ draggableId: t, mode: c });
          var l = { mode: c, draggableId: t, type: n, source: r() },
            u = document.activeElement,
            f =
              u instanceof HTMLElement && u.hasAttribute(L.dragHandle.draggableId)
                ? z(u, L.dragHandle.draggableId)
                : null,
            m = l.source.droppableId,
            h = v.getEntry({ droppableId: m });
          (j(h, `should have entry for droppable '${m}'`),
            (g.current = {
              isDragging: !0,
              mode: c,
              draggableDimensions: F(i),
              restoreFocusTo: f,
              draggableId: t,
              type: n,
              prevDestination: l.source,
              sourceLocation: l.source,
              targetLocation: l.source,
              draggableInitialOffsetInSourceDroppable: Ir({ element: i, mode: h.mode }),
            }),
            o?.(l),
            d.dispatch(`onPendingDragStart`, { start: l, droppable: h }),
            p(function () {
              var e = { mode: c, draggableId: t, type: n, source: r() },
                i = kr(`onDragStart`, e),
                a = i.provided,
                o = i.getMessage;
              (s?.(e, a),
                Tr(o()),
                p(function () {
                  var t = e.source.droppableId,
                    n = v.getEntry({ droppableId: t });
                  n != null &&
                    n.isDropDisabled &&
                    b({ targetLocation: y({ droppableId: t }), isImmediate: !0 });
                }));
            }));
        }
      },
      [v, y, d, a, o, s, p, b],
    ),
    S = re(),
    C = (0, M.useCallback)(
      function (e) {
        var t = e.reason;
        if (g.current.isDragging) {
          (S.runCleanupFn(), t === `CANCEL` && b({ targetLocation: null }));
          var n = g.current,
            r = n.mode,
            i = n.restoreFocusTo,
            a = n.sourceLocation,
            o = n.targetLocation,
            s = n.type,
            c = n.draggableId;
          ((g.current = { isDragging: !1 }), m());
          var f = Xt({ start: a, target: o }),
            p = {
              reason: f === null ? `CANCEL` : `DROP`,
              type: s,
              source: a,
              destination: f,
              mode: r,
              draggableId: c,
              combine: null,
            };
          d.dispatch(`onBeforeDragEnd`, { draggableId: c });
          var h = kr(`onDragEnd`, p),
            _ = h.provided,
            v = h.getMessage;
          (l(p, _),
            Tr(v()),
            i &&
              requestAnimationFrame(function () {
                var e = se({ contextId: u, draggableId: c });
                e && e.focus();
              }));
        }
      },
      [u, m, S, d, l, b],
    ),
    w = (0, M.useMemo)(
      function () {
        return { getDragState: _, startDrag: x, updateDrag: b, stopDrag: C };
      },
      [_, x, C, b],
    );
  nr({ dragController: w, contextId: u });
  var T = ln({
      dragController: w,
      droppableRegistry: v,
      contextId: u,
      setKeyboardCleanupFn: S.setCleanupFn,
    }).startKeyboardDrag,
    E = (0, M.useCallback)(
      function (e) {
        var t = g.current;
        t.isDragging &&
          e.isDropDisabled &&
          e.droppableId === t.targetLocation?.droppableId &&
          b({ targetLocation: y({ droppableId: e.droppableId }) });
      },
      [y, b],
    );
  return (
    v.setUpdateListener(E),
    lr({ contextId: u, nonce: i }),
    M.createElement(
      qt,
      { contextId: u, dragController: w },
      M.createElement(
        yr,
        { lifecycle: d },
        M.createElement(
          ee,
          { contextId: u, getDragState: _, startKeyboardDrag: T, droppableRegistry: v },
          t,
        ),
      ),
    )
  );
}
var Rr = (function () {
  if (typeof window > `u`) return null;
  var e = new Image();
  return (
    (e.src = `data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==`),
    e
  );
})();
function zr(e) {
  var t = e.nativeSetDragImage;
  t && Rr && t(Rr, 0, 0);
}
var Br = (0, M.createContext)(null),
  Vr = Br.Provider;
function Hr() {
  var e = (0, M.useContext)(Br);
  return (j(e, `Missing <Droppable /> parent`), e);
}
function Ur() {
  var e = (0, M.useContext)(Br);
  return e ? e.droppableId : null;
}
function Wr(e) {
  var t = e.elementRef,
    n = e.data,
    r = e.direction,
    i = e.contextId,
    a = e.isDropDisabled,
    o = e.type;
  (0, M.useEffect)(
    function () {
      var e = t.current;
      return (
        j(e instanceof HTMLElement),
        Vt({
          element: e,
          getIsSticky: function () {
            return !0;
          },
          canDrop: function (e) {
            var t = e.source;
            return !H(t.data) || a ? !1 : t.data.type === o && t.data.contextId === i;
          },
          getData: function (t) {
            var i = t.input;
            return he(n, {
              element: e,
              input: i,
              allowedEdges: r === `vertical` ? [`top`, `bottom`] : [`left`, `right`],
            });
          },
        })
      );
    },
    [n, r, i, a, o, t],
  );
}
function Gr() {
  return { startKeyboardDrag: P().startKeyboardDrag };
}
function Kr() {
  return B({ attribute: R.dropIndicator });
}
function qr(e) {
  return B({ attribute: L.placeholder.contextId, value: e });
}
function Jr(e) {
  var t = (0, M.useRef)(e);
  return (
    (0, M.useEffect)(
      function () {
        t.current = e;
      },
      [e],
    ),
    (0, M.useCallback)(function () {
      return t.current;
    }, [])
  );
}
var Yr = { dragging: 5e3 };
function Xr(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function Zr(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? Xr(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : Xr(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var Qr = { transform: void 0, transition: void 0 },
  $r = {
    position: `fixed`,
    top: 0,
    left: 0,
    boxSizing: `border-box`,
    transition: `none`,
    zIndex: Yr.dragging,
    opacity: 0.75,
    pointerEvents: `none`,
  };
function ei(e) {
  var t = e.draggableDimensions,
    n = e.previewOffset,
    r = t.rect,
    i = r.left + n.x,
    a = r.top + n.y,
    o = i === 0 && a === 0;
  return Zr(
    Zr({}, $r),
    {},
    { transform: o ? void 0 : `translate(${i}px, ${a}px)`, width: r.width, height: r.height },
  );
}
function ti(e) {
  var t = e.draggableDimensions,
    n = e.draggableState;
  return n.type !== `dragging` || !n.previewOffset || !t
    ? Qr
    : ei({ draggableDimensions: t, previewOffset: n.previewOffset });
}
var ni = {
  input: !0,
  button: !0,
  textarea: !0,
  select: !0,
  option: !0,
  optgroup: !0,
  video: !0,
  audio: !0,
};
function ri(e, t) {
  if (t == null) return !1;
  if (ni[t.tagName.toLowerCase()]) return !0;
  var n = t.getAttribute(`contenteditable`);
  return n === `true` || n === `` ? !0 : t === e ? !1 : ri(e, t.parentElement);
}
function ii(e, t) {
  var n = t.target;
  return n instanceof HTMLElement ? ri(e, n) : !1;
}
function ai(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function oi(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? ai(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : ai(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var si = (0, M.memo)(
    (0, M.forwardRef)(function (e, t) {
      var n = e.style,
        a = I(),
        o = P().contextId,
        s = i({}, L.placeholder.contextId, o),
        c = (0, M.useMemo)(
          function () {
            if (a) {
              var e = a.margin,
                t = a.rect;
              return oi(
                { boxSizing: `border-box`, width: t.width, height: t.height, margin: e },
                n,
              );
            }
          },
          [a, n],
        );
      return M.createElement(`div`, r({ ref: t, style: c }, s));
    }),
  ),
  Q = {
    vertical: {
      mainAxis: {
        name: `y`,
        offset: `offsetTop`,
        length: `offsetHeight`,
        scrollOffset: `scrollTop`,
        forwardEdge: `bottom`,
        overflow: `overflowY`,
        style: { length: `height`, transform: `translateY` },
      },
      crossAxis: {
        name: `x`,
        offset: `offsetLeft`,
        length: `offsetWidth`,
        style: { length: `width`, offset: `left` },
      },
    },
    horizontal: {
      mainAxis: {
        name: `x`,
        offset: `offsetLeft`,
        length: `offsetWidth`,
        scrollOffset: `scrollLeft`,
        forwardEdge: `right`,
        overflow: `overflowX`,
        style: { length: `width`, transform: `translateX` },
      },
      crossAxis: {
        name: `y`,
        offset: `offsetTop`,
        length: `offsetHeight`,
        style: { length: `height`, offset: `top` },
      },
    },
  };
function ci(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function $(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? ci(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : ci(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var li = { type: `idle`, draggingOver: null };
function ui(e) {
  return { type: `hiding`, draggingOver: null, mode: e };
}
var di = {
  initial: function (e) {
    var t = Q[e.direction],
      n = t.mainAxis,
      r = t.crossAxis;
    return i(i({}, n.name, 0), r.name, 24);
  },
  home: function (e) {
    var t = e.droppable.direction,
      n = e.placeholderRect,
      r = e.draggableDimensions;
    j(n, `the placeholder should exist if in home position`);
    var a = { x: n.x - r.rect.x, y: n.y - r.rect.y },
      o = Q[t],
      s = o.mainAxis,
      c = o.crossAxis;
    return i(i({}, s.name, a[s.name]), c.name, a[c.name] + 24);
  },
  away: function (e) {
    var t = e.droppable.direction,
      n = e.dropIndicatorRect,
      r = e.draggableDimensions;
    j(n, `the drop indicator should exist if in away position`);
    var a = { x: n.x - r.rect.x, y: n.y - r.rect.y },
      o = Q[t],
      s = o.mainAxis,
      c = o.crossAxis;
    return i(i({}, s.name, a[s.name] - 0.5 * r.rect[s.style.length]), c.name, a[c.name] + 24);
  },
};
function fi(e, t) {
  var n = t.update,
    r = t.droppable,
    i = t.draggableDimensions,
    a = t.placeholderRect,
    o = t.dropIndicatorRect;
  if (!r || !i) return e;
  var s = { droppable: r, draggableDimensions: i, placeholderRect: a, dropIndicatorRect: o },
    c = Oe(n.source, n.destination ?? null) ? di.home(s) : di.away(s);
  return c ? $($({}, e), {}, { previewOffset: c }) : e;
}
function pi(e, t) {
  var n = t.start,
    r = t.previewOffset;
  return (
    j(e.type === `idle`, `The draggable is idle.`),
    {
      type: `dragging`,
      draggingOver: n.source.droppableId,
      location: null,
      start: n.source,
      draggableId: n.draggableId,
      mode: n.mode,
      previewOffset: r,
    }
  );
}
function mi(e, t) {
  if (t.type === `START_POINTER_DRAG`)
    return pi(e, $($({}, t.payload), {}, { previewOffset: { x: 0, y: 0 } }));
  if (t.type === `START_KEYBOARD_DRAG`) {
    var n = t.payload,
      r = n.draggableDimensions,
      i = n.droppable;
    return pi(
      e,
      $(
        $({}, t.payload),
        {},
        { previewOffset: di.initial({ draggableDimensions: r, direction: i.direction }) },
      ),
    );
  }
  if (t.type === `UPDATE_DRAG`) {
    j(e.type === `dragging`, `The draggable is dragging.`);
    var a = t.payload.update,
      o = a.destination ? a.destination.droppableId : null;
    return o === e.draggingOver ? e : $($({}, e), {}, { draggingOver: o });
  }
  if (t.type === `UPDATE_POINTER_PREVIEW`) {
    j(e.type === `dragging`, `The draggable is dragging.`);
    var s = t.payload.pointerLocation;
    return $(
      $({}, e),
      {},
      {
        previewOffset: {
          x: s.current.input.clientX - s.initial.input.clientX,
          y: s.current.input.clientY - s.initial.input.clientY,
        },
      },
    );
  }
  return t.type === `UPDATE_KEYBOARD_PREVIEW`
    ? (j(e.type === `dragging`, `The draggable is dragging.`),
      e.type === `dragging` ? fi(e, t.payload) : e)
    : t.type === `DROP`
      ? (j(e.type === `dragging`, `The draggable is dragging.`), li)
      : t.type === `START_HIDING`
        ? (j(e.type === `idle` || e.type === `hiding`), ui(t.payload.mode))
        : t.type === `STOP_HIDING`
          ? (j(e.type === `hiding`), li)
          : e;
}
function hi(e) {
  var t = e.draggingOver,
    n = e.isClone,
    r = e.isDragging,
    i = e.mode;
  return (0, M.useMemo)(
    function () {
      return {
        isClone: n,
        isDragging: r,
        draggingOver: t,
        mode: i,
        isDropAnimating: !1,
        dropAnimation: null,
        combineWith: null,
        combineTargetFor: null,
      };
    },
    [t, n, r, i],
  );
}
var gi = function () {};
function _i(e) {
  var t = e.children,
    n = e.draggableId,
    r = e.index,
    a = e.isDragDisabled,
    o = a === void 0 ? !1 : a,
    s = e.disableInteractiveElementBlocking,
    c = s === void 0 ? !1 : s,
    l = Hr(),
    u = l.direction,
    f = l.droppableId,
    p = l.type,
    m = l.mode,
    g = P(),
    _ = g.contextId,
    v = g.getDragState,
    y = (0, M.useRef)(null),
    b = (0, M.useRef)(null),
    x = re(),
    S = x.setCleanupFn,
    C = x.runCleanupFn,
    w = (0, M.useCallback)(
      function (e) {
        (y.current && C(),
          e && S(ie(e, i(i({}, R.draggable.droppableId, f), R.draggable.index, String(r)))),
          (y.current = e),
          (b.current = se({ contextId: _, draggableId: n })));
      },
      [_, n, f, r, C, S],
    ),
    T = Jr(r),
    E = h((0, M.useReducer)(mi, li), 2),
    D = E[0],
    O = E[1],
    k = ve({ draggableId: n, droppableId: f, getIndex: T, contextId: _, type: p }),
    A = D.type === `dragging`,
    N = D.type === `hiding`,
    ee = Hr(),
    F = ee.shouldRenderCloneWhileDragging,
    te = ee.isDropDisabled,
    ne = br(),
    z = Gr().startKeyboardDrag;
  ((0, M.useEffect)(
    function () {
      if (D.type === `idle` && !o) {
        var e = y.current;
        d(e instanceof HTMLElement);
        var t = b.current;
        return (
          d(t instanceof HTMLElement),
          (0, G.bindAll)(t, [
            {
              type: `keydown`,
              listener: function (t) {
                if (t.key === ` `) {
                  if (t.defaultPrevented || (!c && ii(e, t))) return;
                  (t.preventDefault(),
                    z({
                      event: t,
                      draggableId: n,
                      type: p,
                      getSourceLocation: function () {
                        return { droppableId: f, index: T() };
                      },
                      sourceElement: e,
                    }));
                }
              },
            },
          ])
        );
      }
    },
    [c, n, f, T, o, z, D.type, p],
  ),
    (0, M.useEffect)(
      function () {
        if (!N && !o) {
          var e = y.current;
          j(e instanceof HTMLElement);
          var t = b.current;
          return (
            j(t instanceof HTMLElement),
            Ut({
              canDrag: function (e) {
                var n = e.input;
                return n.ctrlKey || n.metaKey || n.shiftKey || n.altKey
                  ? !1
                  : c
                    ? !A
                    : !ri(t, Ne({ x: n.clientX, y: n.clientY }));
              },
              element: e,
              dragHandle: t,
              getInitialData: function () {
                return k;
              },
              onGenerateDragPreview: zr,
            })
          );
        }
      },
      [k, c, o, A, N],
    ));
  var ae = D.type !== `idle` && m === `standard`,
    B = (0, M.useRef)(null);
  Wr({ elementRef: ae ? B : y, data: k, direction: u, contextId: _, isDropDisabled: te, type: p });
  var oe = (0, M.useRef)(!0);
  ((0, M.useEffect)(function () {
    return (
      (oe.current = !0),
      function () {
        oe.current = !1;
      }
    );
  }, []),
    (0, M.useEffect)(
      function () {
        var e = v();
        F &&
          e.isDragging &&
          e.draggableId === k.draggableId &&
          O({ type: `START_HIDING`, payload: { mode: e.mode } });
      },
      [k.draggableId, v, F],
    ));
  var V = I();
  (0, M.useEffect)(
    function () {
      return F
        ? ne({
            onPendingDragStart: function (e) {
              var t = e.start;
              k.draggableId === t.draggableId &&
                O({ type: `START_HIDING`, payload: { mode: t.mode } });
            },
            onBeforeDragEnd: function (e) {
              e.draggableId === k.draggableId && O({ type: `STOP_HIDING` });
            },
          })
        : U(
            ne({
              onPendingDragStart: function (e) {
                var t = e.start,
                  n = e.droppable;
                if (k.draggableId === t.draggableId) {
                  if (t.mode === `FLUID`)
                    return O({ type: `START_POINTER_DRAG`, payload: { start: t } });
                  if (t.mode === `SNAP`) {
                    var r = v();
                    return (
                      j(r.isDragging && r.draggableDimensions),
                      O({
                        type: `START_KEYBOARD_DRAG`,
                        payload: {
                          start: t,
                          draggableDimensions: r.draggableDimensions,
                          droppable: n,
                        },
                      })
                    );
                  }
                }
              },
              onPendingDragUpdate: function (e) {
                var t = e.update,
                  n = e.droppable;
                k.draggableId === t.draggableId &&
                  (O({ type: `UPDATE_DRAG`, payload: { update: t } }),
                  t.mode === `SNAP` &&
                    queueMicrotask(function () {
                      if (v().isDragging) {
                        var e = qr(_),
                          r = e ? e.getBoundingClientRect() : null,
                          i = Kr();
                        O({
                          type: `UPDATE_KEYBOARD_PREVIEW`,
                          payload: {
                            update: t,
                            draggableDimensions: V,
                            droppable: n,
                            placeholderRect: r,
                            dropIndicatorRect: i ? i.getBoundingClientRect() : null,
                          },
                        });
                      }
                    }));
              },
              onBeforeDragEnd: function (e) {
                e.draggableId === k.draggableId &&
                  (j(oe.current, `isMounted onBeforeDragEnd`), O({ type: `DROP` }));
              },
            }),
            Ht({
              canMonitor: function (e) {
                var t = e.source;
                return H(t.data)
                  ? t.data.contextId === k.contextId && t.data.draggableId === k.draggableId
                  : !1;
              },
              onDrag: function (e) {
                var t = e.location;
                O({ type: `UPDATE_POINTER_PREVIEW`, payload: { pointerLocation: t } });
              },
            }),
          );
    },
    [k.draggableId, k.contextId, ne, F, u, _, V, v],
  );
  var ce = (0, M.useMemo)(
      function () {
        return {
          draggableProps: i(
            i(i({}, L.draggable.contextId, _), L.draggable.id, n),
            `style`,
            ti({ draggableDimensions: V, draggableState: D }),
          ),
          dragHandleProps: i(
            i(
              i(
                i(
                  i({ role: `button`, "aria-describedby": Zt(_) }, L.dragHandle.contextId, _),
                  L.dragHandle.draggableId,
                  n,
                ),
                `tabIndex`,
                0,
              ),
              `draggable`,
              !1,
            ),
            `onDragStart`,
            gi,
          ),
          innerRef: w,
        };
      },
      [_, n, V, D, w],
    ),
    le = hi({ draggingOver: D.draggingOver, isClone: !1, isDragging: A, mode: A ? D.mode : null }),
    ue = (0, M.useMemo)(
      function () {
        return { draggableId: n, type: p, source: { droppableId: f, index: r } };
      },
      [n, f, r, p],
    );
  return M.createElement(
    M.Fragment,
    null,
    N ? null : t(ce, le, ue),
    ae && M.createElement(si, { ref: B }),
  );
}
function vi() {
  return document.body;
}
function yi(e) {
  var t = e.children,
    n = e.droppableId,
    r = e.type,
    a = e.draggableId,
    o = e.index,
    s = e.draggingOver,
    c = e.style,
    l = e.getContainerForClone,
    u = l === void 0 ? vi : l,
    d = e.mode,
    f = P().contextId,
    p = (0, M.useCallback)(
      function (e) {
        e && se({ contextId: f, draggableId: a })?.focus();
      },
      [f, a],
    );
  return (0, X.createPortal)(
    t(
      (0, M.useMemo)(
        function () {
          return {
            innerRef: p,
            draggableProps: i(i(i({}, L.draggable.contextId, f), L.draggable.id, a), `style`, c),
            dragHandleProps: i(
              i(
                i(
                  i(
                    i({ role: `button`, "aria-describedby": Zt(f) }, L.dragHandle.contextId, f),
                    L.dragHandle.draggableId,
                    a,
                  ),
                  `tabIndex`,
                  0,
                ),
                `draggable`,
                !1,
              ),
              `onDragStart`,
              function () {},
            ),
          };
        },
        [f, a, p, c],
      ),
      hi({ draggingOver: s, isClone: !0, isDragging: !0, mode: d }),
      (0, M.useMemo)(
        function () {
          return { draggableId: a, type: r, source: { droppableId: n, index: o } };
        },
        [a, n, o, r],
      ),
    ),
    u(),
  );
}
function bi(e) {
  var t = e.children,
    n = e.droppableId,
    r = e.type,
    i = e.getContainerForClone,
    a = P(),
    o = a.contextId,
    s = a.getDragState,
    c = I(),
    l = h((0, M.useReducer)(mi, li), 2),
    u = l[0],
    d = l[1],
    f = br();
  if (
    ((0, M.useEffect)(
      function () {
        return U(
          f({
            onPendingDragStart: function (e) {
              var t = e.start,
                r = e.droppable;
              if (n === t.source.droppableId) {
                if (t.mode === `FLUID`)
                  return d({ type: `START_POINTER_DRAG`, payload: { start: t } });
                if (t.mode === `SNAP`) {
                  var i = s();
                  return (
                    j(i.isDragging && i.draggableDimensions),
                    d({
                      type: `START_KEYBOARD_DRAG`,
                      payload: {
                        start: t,
                        draggableDimensions: i.draggableDimensions,
                        droppable: r,
                      },
                    })
                  );
                }
              }
            },
            onPendingDragUpdate: function (e) {
              var t = e.update,
                n = e.droppable;
              u.type === `dragging` &&
                u.draggableId === t.draggableId &&
                (d({ type: `UPDATE_DRAG`, payload: { update: t } }),
                t.mode === `SNAP` &&
                  queueMicrotask(function () {
                    if (s().isDragging) {
                      var e = qr(o),
                        r = e ? e.getBoundingClientRect() : null,
                        i = Kr();
                      d({
                        type: `UPDATE_KEYBOARD_PREVIEW`,
                        payload: {
                          update: t,
                          draggableDimensions: c,
                          droppable: n,
                          placeholderRect: r,
                          dropIndicatorRect: i ? i.getBoundingClientRect() : null,
                        },
                      });
                    }
                  }));
            },
            onBeforeDragEnd: function (e) {
              var t = e.draggableId;
              u.type === `dragging` && t === u.draggableId && d({ type: `DROP` });
            },
          }),
          Ht({
            canMonitor: function (e) {
              var t = e.source;
              return H(t.data) ? t.data.contextId === o && t.data.droppableId === n : !1;
            },
            onDrag: function (e) {
              var t = e.location;
              d({ type: `UPDATE_POINTER_PREVIEW`, payload: { pointerLocation: t } });
            },
          }),
        );
      },
      [n, o, f, u, c, s],
    ),
    u.type !== `dragging`)
  )
    return null;
  var p = ti({ draggableDimensions: c, draggableState: u });
  return M.createElement(
    yi,
    {
      droppableId: n,
      type: r,
      draggableId: u.draggableId,
      index: u.start.index,
      draggingOver: u.draggingOver,
      mode: u.mode,
      style: p,
      getContainerForClone: i,
    },
    t,
  );
}
var xi = {
  horizontal: { rect: { start: `left`, end: `right` } },
  vertical: { rect: { start: `top`, end: `bottom` } },
};
function Si(e) {
  var t = e.a,
    n = e.b,
    r = xi[e.direction].rect;
  return Math.max(t[r.start], n[r.start]) - Math.min(t[r.end], n[r.end]);
}
function Ci(e) {
  return z(e, R.draggable.droppableId);
}
function wi(e) {
  var t = z(e, R.draggable.index),
    n = parseInt(t);
  return (j(Number.isInteger(n), `invalid index: '${n}' is not an integer`), n);
}
function Ti(e) {
  var t = e.element,
    n = e.where,
    r = e.direction,
    i = e.contextId,
    a = Ci(t),
    o = wi(t),
    s = o - 1,
    c = o + 1,
    l = nn(i, { droppableId: a, index: n === `before` ? s : c });
  if (l === null) {
    var u = getComputedStyle(t),
      d = u.marginTop,
      f = u.marginRight,
      p = u.marginBottom,
      m = u.marginLeft;
    return r === `horizontal` ? parseFloat(m) + parseFloat(f) : parseFloat(d) + parseFloat(p);
  }
  return Si({ direction: r, a: t.getBoundingClientRect(), b: l.getBoundingClientRect() });
}
function Ei(e) {
  var t = e.element,
    n = e.where,
    r = e.direction,
    i = e.contextId,
    a = Ti({ element: t, where: n, direction: r, contextId: i });
  return n === `before` ? -a / 2 : a / 2;
}
function Di(e) {
  var t = e.element,
    n = e.isForwardEdge,
    r = e.mode,
    i = e.direction,
    a = e.contextId,
    o = Q[i],
    s = o.mainAxis,
    c = o.crossAxis,
    l = le({ element: t, mode: r }),
    u = Ei({ element: t, where: n ? `after` : `before`, direction: i, contextId: a }),
    d = l[s.offset] - 1;
  return {
    mainAxis: { offset: (n ? d + t[s.length] : d) + u },
    crossAxis: { offset: l[c.offset], length: l[c.length] },
  };
}
function Oi(e) {
  var t = e.element,
    n = Q[e.direction],
    r = n.mainAxis,
    i = n.crossAxis;
  return {
    mainAxis: { offset: t[r.offset] - 1 + t[r.length] / 2 },
    crossAxis: { offset: t[i.offset], length: t[i.length] },
  };
}
function ki(e) {
  var t = e.element,
    n = Q[e.direction].mainAxis,
    r = $t(t);
  return !r || getComputedStyle(r).position !== `static` ? 0 : r[n.offset];
}
function Ai(e) {
  var t = e.droppableId,
    n = e.direction;
  return {
    mainAxis: { offset: ki({ element: V({ attribute: L.droppable.id, value: t }), direction: n }) },
    crossAxis: { offset: 0, length: `100%` },
  };
}
function ji(e) {
  var t = e.targetLocation,
    n = e.isInHomeLocation,
    r = e.direction,
    i = e.mode,
    a = e.contextId;
  if (n) {
    var o = qr(a);
    return o ? Oi({ element: o, direction: r }) : null;
  }
  if (t.index === 0) {
    var s = nn(a, t);
    return s
      ? Di({ element: s, isForwardEdge: !1, mode: i, direction: r, contextId: a })
      : Ai({ droppableId: t.droppableId, direction: r });
  }
  var c = nn(a, { droppableId: t.droppableId, index: t.index - 1 });
  return c ? Di({ element: c, isForwardEdge: !0, mode: i, direction: r, contextId: a }) : null;
}
var Mi = 4,
  Ni = o({
    background: `var(--ds-border-brand, #1868DB)`,
    scrollMarginTop: Mi,
    scrollMarginBottom: Mi + 1,
  }),
  Pi = o({ position: `absolute`, top: 0, left: 0 }),
  Fi = o({ opacity: 0 }),
  Ii = {
    horizontal: o({ width: 2, height: `100%`, marginLeft: -2 }),
    vertical: o({ width: `100%`, height: 2, marginTop: -2 }),
  };
function Li(e) {
  var t = e.direction,
    n = e.dimensions,
    r = e.indicatorOffset;
  if (n === null) return { opacity: 0 };
  var a = Q[t],
    o = a.mainAxis,
    s = a.crossAxis;
  return i(
    i(
      { transform: `${o.style.transform}(${n.mainAxis.offset - r}px)` },
      s.style.length,
      n.crossAxis.length,
    ),
    s.style.offset,
    n.crossAxis.offset,
  );
}
var Ri = i({}, R.dropIndicator, ``),
  zi = function (e) {
    var t = e.direction,
      n = e.mode,
      i = P(),
      o = i.contextId,
      s = i.getDragState,
      c = (0, M.useRef)(null),
      l = h((0, M.useState)(null), 2),
      u = l[0],
      d = l[1],
      f = h((0, M.useState)(!1), 2),
      p = f[0],
      m = f[1],
      g = br(),
      _ = (0, M.useCallback)(
        function (e) {
          var r = e.targetLocation,
            i = e.source,
            a = e.destination;
          if (!r) return d(null);
          var s = Oe(i, a);
          return (
            m(s),
            d(ji({ targetLocation: r, isInHomeLocation: s, direction: t, mode: n, contextId: o }))
          );
        },
        [o, t, n],
      );
    ((0, M.useLayoutEffect)(
      function () {
        var e = s();
        if (e.isDragging) {
          var t = e.targetLocation,
            n = e.sourceLocation;
          return (
            _({ targetLocation: t, destination: Xt({ start: n, target: t }), source: n }),
            g({
              onPrePendingDragUpdate: function (e) {
                var t = e.update,
                  n = e.targetLocation,
                  r = t.destination,
                  i = r === void 0 ? null : r,
                  a = t.source;
                _({ targetLocation: n, source: a, destination: i });
              },
            })
          );
        }
      },
      [o, t, s, n, g, _],
    ),
      (0, M.useLayoutEffect)(
        function () {
          if (u !== null) {
            var e = s();
            if (!(!e.isDragging || e.mode !== `SNAP`)) {
              var t = c.current;
              (j(t instanceof HTMLElement), t.scrollIntoView({ block: `nearest` }));
            }
          }
        },
        [u, s],
      ));
    var v = Q[t].mainAxis,
      y = Li({ direction: t, dimensions: u, indicatorOffset: c.current ? c.current[v.offset] : 0 }),
      b = n === `virtual`;
    return a(`div`, r({ ref: c, css: [Ni, Ii[t], b && Pi, p && Fi], style: y }, Ri));
  };
function Bi(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function Vi(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? Bi(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : Bi(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
var Hi = { draggingFromThisWith: null, draggingOverWith: null, isDraggingOver: !1 };
function Ui(e, t) {
  if (t.type === `DRAG_START`) {
    var n = t.payload,
      r = n.droppableId,
      i = n.start,
      a = i.draggableId,
      o = i.source,
      s = o.droppableId === r,
      c = s ? a : null,
      l = o.droppableId === r ? a : null;
    return Vi(Vi({}, e), {}, { isDraggingOver: s, draggingFromThisWith: l, draggingOverWith: c });
  }
  if (t.type === `DRAG_UPDATE`) {
    var u = t.payload,
      d = u.droppableId,
      f = u.update,
      p = f.destination,
      m = p === void 0 ? null : p,
      h = f.draggableId,
      g = f.source,
      _ = m?.droppableId === d,
      v = _ ? h : null,
      y = g.droppableId === d ? h : null;
    return Vi(Vi({}, e), {}, { isDraggingOver: _, draggingFromThisWith: y, draggingOverWith: v });
  }
  return t.type === `DRAG_CLEAR` ? Hi : e;
}
function Wi(e) {
  var t = e.draggableId,
    n = e.droppableId,
    r = e.type,
    i = e.direction,
    a = e.isDropDisabled,
    o = (0, M.useRef)(null),
    s = P(),
    c = s.contextId,
    l = s.getDragState,
    u = l();
  (j(u.isDragging, `The virtual placeholder should only be rendered during a drag`),
    Wr({
      elementRef: o,
      data: ve({
        draggableId: t,
        droppableId: n,
        getIndex: (0, M.useCallback)(
          function () {
            return u.sourceLocation.index;
          },
          [u.sourceLocation.index],
        ),
        contextId: c,
        type: r,
      }),
      direction: i,
      contextId: c,
      isDropDisabled: a,
      type: r,
    }));
  var d = (0, M.useMemo)(
    function () {
      return {
        position: `absolute`,
        top: u.draggableInitialOffsetInSourceDroppable.top,
        left: u.draggableInitialOffsetInSourceDroppable.left,
        margin: 0,
      };
    },
    [u.draggableInitialOffsetInSourceDroppable.left, u.draggableInitialOffsetInSourceDroppable.top],
  );
  return M.createElement(si, { ref: o, style: d });
}
function Gi(e) {
  var t = e.children,
    n = e.droppableId,
    r = e.type,
    a = r === void 0 ? `DEFAULT` : r,
    o = e.direction,
    s = o === void 0 ? `vertical` : o,
    c = e.mode,
    l = c === void 0 ? `standard` : c,
    u = e.renderClone,
    f = e.getContainerForClone,
    p = e.isDropDisabled,
    m = p === void 0 ? !1 : p,
    g = Jr(m),
    _ = P(),
    v = _.contextId,
    y = _.droppableRegistry,
    b = xe({ contextId: v, droppableId: n, getIsDropDisabled: g }),
    x = (0, M.useRef)(null),
    S = (0, M.useCallback)(
      function (e) {
        (e &&
          ie(
            e,
            i(
              i(i(i({}, R.droppable.type, a), R.droppable.direction, s), L.droppable.id, n),
              L.droppable.contextId,
              v,
            ),
          ),
          (x.current = e));
      },
      [v, s, n, a],
    ),
    C = h((0, M.useReducer)(Ui, Hi), 2),
    w = C[0],
    T = C[1],
    E = w.draggingFromThisWith,
    D = w.draggingOverWith,
    O = w.isDraggingOver,
    k = Ur();
  (0, M.useEffect)(
    function () {
      var e = x.current;
      return (
        d(e instanceof HTMLElement, "innerRef must provide an `HTMLElement`"),
        U(
          y.register({
            droppableId: n,
            type: a,
            isDropDisabled: m,
            parentDroppableId: k,
            element: e,
            direction: s,
            mode: l,
          }),
          Vt({
            element: e,
            getData: function (t) {
              var n = t.input;
              return he(b, {
                element: e,
                input: n,
                allowedEdges: s === `vertical` ? [`top`, `bottom`] : [`left`, `right`],
              });
            },
            canDrop: function (e) {
              var t = e.source;
              return !H(t.data) || m ? !1 : t.data.contextId === v && t.data.type === a;
            },
            onDragLeave: function () {
              T({ type: `DRAG_CLEAR` });
            },
          }),
        )
      );
    },
    [b, n, v, m, a, y, k, s, l],
  );
  var A = br();
  (0, M.useEffect)(
    function () {
      function e(e) {
        var t = e.type === a,
          r = e.destination?.droppableId === n;
        return t && ((!O && r) || (O && !r));
      }
      return A({
        onPendingDragStart: function (t) {
          var r = t.start;
          e({ destination: r.source, type: r.type }) &&
            T({ type: `DRAG_START`, payload: { droppableId: n, start: r } });
        },
        onPendingDragUpdate: function (t) {
          var r = t.update;
          e(r) && T({ type: `DRAG_UPDATE`, payload: { droppableId: n, update: r } });
        },
        onBeforeDragEnd: function () {
          T({ type: `DRAG_CLEAR` });
        },
      });
    },
    [n, O, A, a],
  );
  var j = (0, M.useMemo)(
      function () {
        return O ? M.createElement(zi, { direction: s, mode: l }) : null;
      },
      [s, O, l],
    ),
    N = (0, M.useMemo)(
      function () {
        return {
          innerRef: S,
          droppableProps: i(i({}, L.droppable.contextId, v), L.droppable.id, n),
          placeholder: l === `standard` ? j : null,
        };
      },
      [v, j, n, l, S],
    ),
    ee = (0, M.useMemo)(
      function () {
        return {
          draggingFromThisWith: E,
          draggingOverWith: D,
          isDraggingOver: O,
          isUsingPlaceholder: O,
        };
      },
      [E, D, O],
    ),
    F = x.current,
    I = O && l === `virtual` && F;
  rr(
    function () {
      if (I && window.getComputedStyle(F).position === `static`) {
        var e = F.style.position;
        return (
          (F.style.position = `relative`),
          function () {
            F.style.position = e;
          }
        );
      }
    },
    [F, I],
  );
  var te = !!u,
    ne = (0, M.useMemo)(
      function () {
        return {
          direction: s,
          droppableId: n,
          shouldRenderCloneWhileDragging: te,
          isDropDisabled: m,
          type: a,
          mode: l,
        };
      },
      [s, n, te, m, a, l],
    ),
    re = E && l === `virtual` && F;
  return M.createElement(
    Vr,
    { value: ne },
    t(N, ee),
    I && (0, X.createPortal)(j, F),
    re &&
      (0, X.createPortal)(
        M.createElement(Wi, {
          droppableId: n,
          draggableId: E,
          type: a,
          direction: s,
          isDropDisabled: m,
        }),
        F,
      ),
    u && M.createElement(bi, { droppableId: n, type: a, getContainerForClone: f }, u),
  );
}
m();
var Ki = [`isRanking`, `isRankingItem`, `testId`],
  qi = (0, M.forwardRef)(function (e, t) {
    var n = e.isRanking,
      i = e.isRankingItem,
      a = e.testId,
      o = s(e, Ki);
    return M.createElement(
      f,
      r({ ref: t, testId: a }, o, {
        className: p([
          n && `_1e0c1ule`,
          i && `_2rkolb4i _bfhk1bhr _16qs130s`,
          `_12y3e4h9 _mizu1v1w _ra3xnqa1`,
        ]),
      }),
    );
  });
m();
var Ji = [`isRanking`, `innerRef`],
  Yi = function (e) {
    var t = e.isRanking,
      n = e.innerRef,
      i = s(e, Ji);
    return M.createElement(v, r({}, i, { innerRef: n, className: p([t && `_vchhusvi`]) }));
  },
  Xi = [`content`, `testId`];
function Zi(e, t, n) {
  return ((t = u(t)), w(e, Qi() ? Reflect.construct(t, n || [], u(e).constructor) : t.apply(e, n)));
}
function Qi() {
  try {
    var e = !Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {}));
  } catch {}
  return (Qi = function () {
    return !!e;
  })();
}
var $i = y(
    (function (e) {
      function t() {
        return (l(this, t), Zi(this, t, arguments));
      }
      return (
        T(t, e),
        C(t, [
          {
            key: `render`,
            value: function () {
              var e = this.props,
                t = e.cell,
                n = e.head,
                i = e.isFixedSize,
                a = e.isRanking,
                o = e.refWidth,
                c = e.innerRef,
                l = e.testId,
                u = t.content,
                d = t.testId,
                f = s(t, Xi),
                p = n || {},
                m = p.shouldTruncate,
                h = p.width,
                g = _(a, o);
              return M.createElement(
                Yi,
                r({}, f, {
                  isFixedSize: i,
                  shouldTruncate: m,
                  width: h,
                  isRanking: a,
                  style: g,
                  onKeyDown: function (e) {
                    return e.stopPropagation();
                  },
                  innerRef: c,
                  "data-testid": d || (l && `${l}--rankable--table--body--cell`),
                }),
                u,
              );
            },
          },
        ])
      );
    })(M.Component),
  ),
  ea = [`cells`, `testId`, `key`, `isHighlighted`];
function ta(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    (t &&
      (r = r.filter(function (t) {
        return Object.getOwnPropertyDescriptor(e, t).enumerable;
      })),
      n.push.apply(n, r));
  }
  return n;
}
function na(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] == null ? {} : arguments[t];
    t % 2
      ? ta(Object(n), !0).forEach(function (t) {
          i(e, t, n[t]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
        : ta(Object(n)).forEach(function (t) {
            Object.defineProperty(e, t, Object.getOwnPropertyDescriptor(n, t));
          });
  }
  return e;
}
function ra(e, t, n) {
  return ((t = u(t)), w(e, ia() ? Reflect.construct(t, n || [], u(e).constructor) : t.apply(e, n)));
}
function ia() {
  try {
    var e = !Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {}));
  } catch {}
  return (ia = function () {
    return !!e;
  })();
}
var aa = y(
  (function (e) {
    function t() {
      var e;
      l(this, t);
      var n = [...arguments];
      return (
        (e = ra(this, t, [].concat(n))),
        i(e, `innerRef`, function (t) {
          return function (n) {
            (t(n), typeof e.props.innerRef == `function` && e.props.innerRef(n));
          };
        }),
        e
      );
    }
    return (
      T(t, e),
      C(t, [
        {
          key: `render`,
          value: function () {
            var e = this,
              t = this.props,
              n = t.row,
              i = t.head,
              a = t.isFixedSize,
              o = t.isRanking,
              c = t.refWidth,
              l = t.rowIndex,
              u = t.isRankingDisabled,
              d = t.isHighlighted,
              f = t.testId,
              p = n.cells,
              m = n.testId,
              h = n.key,
              g = n.isHighlighted,
              v = s(n, ea),
              y = _(o, c);
            if (typeof h != `string` && !u)
              throw Error(
                `dynamic-table: ranking is not possible because table row does not have a key. Add the key to the row or disable ranking.`,
              );
            return M.createElement(
              _i,
              { draggableId: h || l.toString(), index: l, isDragDisabled: u },
              function (t, n) {
                return M.createElement(
                  qi,
                  r({}, v, t.dragHandleProps, t.draggableProps, {
                    role: void 0,
                    "aria-labelledby": void 0,
                    "aria-describedby": t.dragHandleProps?.[`aria-describedby`],
                    ref: e.innerRef(t.innerRef),
                    style: na(na({}, t.draggableProps.style), y),
                    isHighlighted: d || g,
                    isRanking: o,
                    isRankingItem: n.isDragging,
                    testId: m || (f && `${f}--rankable--table--body--row`),
                  }),
                  p.map(function (e, t) {
                    var n = (i || { cells: [] }).cells[t];
                    return M.createElement($i, {
                      head: n,
                      cell: e,
                      isRanking: o,
                      key: e.key || t,
                      isFixedSize: a,
                      testId: f,
                    });
                  }),
                );
              },
            );
          },
        },
      ])
    );
  })(M.Component),
);
function oa(e, t, n) {
  return ((t = u(t)), w(e, sa() ? Reflect.construct(t, n || [], u(e).constructor) : t.apply(e, n)));
}
function sa() {
  try {
    var e = !Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {}));
  } catch {}
  return (sa = function () {
    return !!e;
  })();
}
var ca = function (e, t) {
    var n = e.source.index,
      r = e.destination;
    if (r) {
      var i = r.index,
        a = i < n ? i - 1 : i,
        o = a === -1 ? void 0 : t[a].key,
        s = a === -1 ? 0 : a + 1;
      return { index: i, afterKey: o, beforeKey: s < t.length ? t[s].key : void 0 };
    }
  },
  la = (function (e) {
    function t() {
      var e;
      l(this, t);
      var n = [...arguments];
      return (
        (e = oa(this, t, [].concat(n))),
        i(e, `onBeforeDragStart`, function (t) {
          var n = t.draggableId,
            r = { index: t.source.index, key: n };
          e.props.onRankStart(r);
        }),
        i(e, `onDragEnd`, function (t) {
          var n = e.props,
            r = n.pageRows,
            i = n.onRankEnd,
            a = t.draggableId;
          i({ sourceIndex: t.source.index, sourceKey: a, destination: ca(t, r) });
        }),
        e
      );
    }
    return (
      T(t, e),
      C(t, [
        {
          key: `render`,
          value: function () {
            var e = this.props,
              t = e.highlightedRowIndex,
              n = e.pageRows,
              i = e.head,
              a = e.isFixedSize,
              o = e.isRanking,
              s = e.isRankingDisabled,
              c = e.testId,
              l = e.forwardedRef;
            return M.createElement(
              Lr,
              { onBeforeDragStart: this.onBeforeDragStart, onDragEnd: this.onDragEnd },
              M.createElement(
                Gi,
                { droppableId: `dynamic-table-droppable`, isDropDisabled: s },
                function (e) {
                  return M.createElement(
                    `tbody`,
                    r(
                      {
                        "data-testid": c,
                        ref: function (t) {
                          (e && typeof e.innerRef == `function` && e.innerRef(t),
                            l && (l.current = t));
                        },
                      },
                      e.droppableProps,
                    ),
                    n.map(function (e, n) {
                      return M.createElement(aa, {
                        head: i,
                        isRanking: o,
                        isFixedSize: a,
                        key: e.key,
                        rowIndex: n,
                        row: e,
                        isRankingDisabled: s,
                        isHighlighted:
                          t !== void 0 && (typeof t == `number` ? t === n : t.indexOf(n) > -1),
                        testId: c && `${c}--${e.key}--rankable--table--row`,
                      });
                    }),
                    e.placeholder,
                  );
                },
              ),
            );
          },
        },
      ])
    );
  })(M.Component),
  ua = g(
    M.forwardRef(function (e, t) {
      return M.createElement(la, r({}, e, { forwardedRef: t }));
    }),
  );
export { ua as default };
