...
import { SQLWrapper, WithSubquery } from "drizzle-orm";
import { Rovo } from "../rovo";
import { ForgeSQLCacheOperations } from "../cache/ForgeSQLCacheOperations";
import { normalizeFunction } from "../sql";
 ...
 */
class ForgeSQLORMCacheImpl implements ForgeSqlOperationExt {
  private static instance: ForgeSqlOperationExt | null = null;
  ...
  private readonly forgeSQLORM: ForgeSqlOperation;
  private readonly options: ForgeSqlOrmOptions;
  private readonly cacheOperations: ForgeSQLCacheOperations & VerioningModificationForgeSQL;
  ...
  private constructor(options?: ForgeSqlOrmOptionsExtra) {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- MySqlRemoteDatabase's schema generic is a Drizzle internal type; this driver is schema-agnostic at this layer
      this.drizzle = this.forgeSQLORM.getDrizzleQueryBuilder() as MySqlRemoteDatabase<any> & {
        selectAliased: SelectAliasedType;
        selectAliasedDistinct: SelectAliasedDistinctType;
        executeQuery: ExecuteQuery;
        selectAliasedCacheable: SelectAliasedCacheableType;
  ...
  async executeWithLocalCacheContextAndReturnValue<T>(cacheContext: () => Promise<T>): Promise<T> {
    return this.forgeSQLORM.executeWithLocalCacheContextAndReturnValue(cacheContext);
  }
  ...
  executeWithLocalContext(cacheContext: () => Promise<void>): Promise<void> {
    return this.forgeSQLORM.executeWithLocalContext(cacheContext);
  }
  /**
  ...
  insert<TTable extends MySqlTable>(
    ...
    return this.forgeSQLORM.insert(table);
  }
  /**
  ...
  update<TTable extends MySqlTable>(
    ...
    return this.forgeSQLORM.update(table);
  }
  ...
  delete<TTable extends MySqlTable>(
    ...
    return this.forgeSQLORM.delete(table);
  }
  /**
   ...
   * @returns The singleton instance of ForgeSQLORMImpl.
   */
  static getInstance(options?: ForgeSqlOrmOptions): ForgeSqlOperationExt {
    ForgeSQLORMCacheImpl.instance ??= new ForgeSQLORMCacheImpl(options);
    return ForgeSQLORMCacheImpl.instance;
  }
  ...
  analyze(): SchemaAnalyzeForgeSql {
    return this.forgeSQLORM.analyze();
  }
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- MySqlRemoteDatabase's schema generic is a Drizzle internal type; this driver is schema-agnostic at this layer
  getDrizzleQueryBuilder(): MySqlRemoteDatabase<any> & {
    selectAliased: SelectAliasedType;
    selectAliasedDistinct: SelectAliasedDistinctType;
    executeQuery: ExecuteQuery;
    selectAliasedCacheable: SelectAliasedCacheableType;
  ...
  select<TSelection extends SelectedFields>(
    ...
    return this.forgeSQLORM.select(fields);
  }
  ...
  selectDistinct<TSelection extends SelectedFields>(
    ...
    return this.forgeSQLORM.selectDistinct(fields);
  }
  ...
  selectFrom<T extends MySqlTable>(table: T) {
    return this.forgeSQLORM.selectFrom(table);
  }
  ...
  selectDistinctFrom<T extends MySqlTable>(table: T) {
    return this.forgeSQLORM.selectDistinctFrom(table);
  }
  ...
  execute<T>(query: SQLWrapper | string) {
    return this.forgeSQLORM.execute<T>(query);
  }
  ...
  async executeDDLActions<T>(actions: () => Promise<T>): Promise<T> {
    return this.forgeSQLORM.executeDDLActions(actions);
  }
  ...
  with(...queries: WithSubquery[]) {
    return this.forgeSQLORM.with(...queries);
  }
  ...
  evictCache(tables: string[]): Promise<void> {
    return this.cacheOperations.evictCache(tables);
  }

  evictCacheEntities(tables: AnyMySqlTable[]): Promise<void> {
    return this.cacheOperations.evictCacheEntities(tables);
  }
}
...
class ForgeSQLORMExt implements ForgeSqlOperationExt {
  ...
  evictCache(tables: string[]): Promise<void> {
    return this.ormInstance.evictCache(tables);
  }

  evictCacheEntities(tables: AnyMySqlTable[]): Promise<void> {
    return this.ormInstance.evictCacheEntities(tables);
  }
}

export default ForgeSQLORMExt;
